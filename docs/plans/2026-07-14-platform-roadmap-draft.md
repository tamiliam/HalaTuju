...
> **⚠ PARTIALLY SUPERSEDED 2026-07-26 by the PROGRAMME LAYER** — see `docs/plans/2026-07-26-programme-layer-roadmap.md` (owner-approved model; decision records in `docs/decisions.md` dated 2026-07-26). This roadmap assumed a tenant runs exactly ONE programme. The owner's model is **Organisation → Programme → Year → award**: BrightPath Sabah is a second *Programme* under BrightPath (not a second Organisation), restricted funds and sponsor acceptance attach to the Programme, and reviewers may be programme-scoped (NULL = org-wide). **What changes here:** Phase 3 S10a/S11 (superadmin portal) move OUT of the critical path — both incoming tenants are hand-configured; S10b survives as the module-flag reconciliation; Phase 2 S7–S9 now gate Inspire (take the split-gate option); **Sprint E erasure is promoted from conditional to HARD-BLOCKING** (Inspire is a separate legal entity → DPA). Phases 1, 5 and 6 stand as written. Work from the programme-layer roadmap first; this file remains the reference for the org fence, module model and migration strategy.
