...
- Nothing broke. Two near-misses worth recording:
  - **The dropped 48h delay would have shipped a visible bug.** During an embargo,
    ...
    for two days, while their uploads silently 403'd. The embargo has never run in production
    (`DECLINE_COOLOFF_DAYS=0`), so this path had never been exercised. It is now moot here, but the
    masking is still hardcoded to `'interviewed'` for any future embargoed decline — **it should
    mask to `pre_decline_status`, which is already snapshotted.** Not fixed this sprint (out of
    scope, and nothing live triggers it); recorded here as the detecting note.
