...
- 2026-07-29 fix (found in passing, prod bookkeeping only): migration `0135_application_catalogue` had its TABLES created on prod but its `django_migrations` row never recorded, so the ledger read 0134 → (nothing) → 0136. Schema and 19 catalogue rows were correct throughout; the row is now recorded stamped to the commit time. Worth a guardrail — a `showmigrations`-vs-prod check at sprint close would have caught it (candidate for the next consolidation review)
