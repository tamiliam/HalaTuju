...
| ✅ **D. Gemini v2 refines with findings** (1 sprint) — **DONE v2.18.0 (2026-05-31)** | Second Gemini call (`refine_sponsor_profile`): draft + the submitted interview's findings/rubric/note → `SponsorProfile.final_markdown` (the field name landed as `finalised_at`, not `profile_finalised_at`). **Admin-facing only** until Phase E gives the sponsor a reader (TD-067). | Adds ~$0.001/applicant. Real payoff — the interview work compounds into the sponsor artefact. |
