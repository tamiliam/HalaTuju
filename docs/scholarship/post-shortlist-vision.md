...
|---|---|---|
| **A. Deterministic anomaly engine** (1 sprint) | Rule-based flag list shown on `/admin/scholarship/[id]` as "Pre-interview flags." Each flag has a static suggested question. No capture yet. | Cheap (no new Gemini calls). Validates flag usefulness before building capture infrastructure. Admin can mentally use it tomorrow. |
| **C. Admin role categories + InterviewSession + capture UI** (1-2 sprints) | New `role` on PartnerAdmin + `assigned_to` on Application. New `InterviewSession` model with structured `findings: JSONField`. Admin UI: list view filtered by "assigned to me" + per-application interview form. New status `interviewed` between `shortlisted` and `accepted`. | Biggest single piece. Worth building only after Phase A confirms the flags are useful. Form schema must lock the structured per-flag verdicts. |
| **B. Gemini-derived gaps** (1 sprint) | Extend `_build_prompt` to ask Gemini to also output `gaps: [{question, why}]`. Merge with deterministic flags into the unified agenda. | One extra prompt-param, same call. Lower priority — the deterministic engine likely covers 70-80% of the value. |
| **D. Gemini v2 refines with findings** (1 sprint) | Second Gemini call: draft + structured findings → final profile. Sponsor sees v2 only. New `profile_finalised_at` field. | Adds ~$0.001/applicant. Real payoff — the interview work compounds into the sponsor artefact. |
