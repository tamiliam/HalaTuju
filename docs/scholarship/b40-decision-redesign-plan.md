...
- **S12b — GATED DEPLOY (the only deploy).** — ✅ **DEPLOYED 2026-05-25.** Migrated prod first (courses 0048 + scholarship 0007–0009, zero-downtime/additive), merged `feature/b40-redesign` → `main` (release `55c2c36`), both services deployed (SUCCESS, health 200), live site unaffected. Cohort `b40-2026` live + thresholds corrected to settled S8 (legacy 5/3.0 → 4/2.9). Idempotent `seed_b40_2026_cohort` command added. **Note:** Cloud Run triggers don't run migrations (manual migrate-first). **Cloud Scheduler DEFERRED** (no applicants while unpromoted; wire before promoting). See `retrospective-b40-sprint12b.md`.
- **S13 — Vision OCR (post-launch fast-follow).** MyKad upload → instant NRIC match feedback (soft, never a hard block), surfaced to admin. New Google Cloud Vision API key + **cost sign-off** before paid calls. **Also wire Cloud Scheduler → `send_pending_decision_emails` before promoting** (carried over from S12b).
- Acceptance: responsive verified (S12a ✅); deployed + cohort live + healthy (S12b ✅, scheduler pre-promote TODO); Vision feedback works (S13).
