...
**Alternatives considered:** (a) The 48h email embargo as originally requested — rejected by the owner once the lockout was shown to be independent of it; it would also have been the first live use of the embargo's student-status MASKING (`ApplicationReadSerializer.get_status` returns 'interviewed' during an embargo), which for a shortlisted student would have shown a stage they never reached, while their uploads silently 403'd. (b) Reuse `AdminRejectView`/`admin_reject('interview')` with `cooloff=timedelta(0)` — rejected: that endpoint's `_require_app_write` also admits a `qc` and the assigned reviewer, and the 'interview' decline copy ("thank you for COMPLETING your application… we were only able to support the students who most closely met our criteria") is false for a student who never completed.
