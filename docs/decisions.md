...
## Award-email installation guide sourced LIVE from Drive (cached + fallback) — 2026-07-21
**Decision:** `emails.vircle_guide_attachment()` fetches the Vircle eWallet installation guide at send time from the owner's Drive folder `03 Vircle/05 Student Guide/` (via the read-only `sheets.fetch_drive_pdf`), caches the bytes briefly (`VIRCLE_GUIDE_CACHE_SECONDS`=600), and falls back to the bundled repo asset if Drive is disabled/unreachable. Drive is the source of truth; the folder/filename/TTL are env-overridable settings.
**Alternatives considered:** (a) One-time sync — download the Drive PDF and overwrite the committed repo asset, keep attaching the bundle. Rejected by the owner: every future edit would need a re-sync + redeploy. (b) `drive.readonly` scope for least privilege — rejected: it is NOT in this SA's domain-wide-delegation allowlist (only full `drive` is), so it would fail `unauthorized_client`; reusing the already-granted `drive` scope is what works and needs no Workspace-admin change. (c) No cache (always fetch fresh) — viable and simpler to reason about, but a batch send would re-download ~1.5 MB per email; a short TTL gives near-freshness (≤10 min) with one download per batch.
**Rationale:** the guide changes independently of code; sourcing it live removes the "edit → re-sync → redeploy" loop the owner explicitly wanted gone. The SA already has access to the `03 Vircle` tree (the payments filer writes there), verified by a read-only probe before coding.
**Trade-offs:** an award email now has a soft runtime dependency on Drive (mitigated: best-effort, never raises, bundled fallback); an owner edit reflects only after the ≤10-min cache window; the bundled asset must be kept as the offline fallback.
**Revisit if:** the guide needs to be instant-on-edit (set TTL to 0), the SA loses folder access, or award-email volume grows enough that the per-batch download matters (raise the TTL).

## STPM submission gate: exempt STPM from the offer-genuineness block, keep presence — Sprint 2026-07-20
**Decision:** `_offer_blocks` (the submission/consent gate) returns False when `chosen_pathway == 'stpm'`, so an STPM student is never barred at submission by `offer_not_official`. The offer document must still be PRESENT (`offer_letter_missing` unchanged). Only STPM is exempt.
**Alternatives considered:** (a) Train the doc-recognition model to accept a school enrolment letter (*Surat Pengesahan Pelajar*) as a genuine STPM offer — rejected for now: no fixed public-issuer signature to anchor on (every school's letterhead differs), so it's a weak, forgeable anchor; it needs a MODEL_VERSION bump + cohort re-run for a gate a human reviewer already backstops. (b) Make the offer fully OPTIONAL for STPM (drop presence too) — rejected: presence gives the reviewer a document to audit and costs the genuine student nothing (they have the letter). (c) Exempt the whole pre-U family (STPM + Matriculation) — rejected: Matriculation students DO receive a recognisable official offer (Bahagian Matrikulasi), so the genuineness gate correctly applies to them; STPM is the only route where the "offer" is really just school enrolment.
**Rationale:** The owner's need was "don't block STPM students at the door" — a submission-policy gap, not an AI-accuracy gap. Fixing the gate is a one-function change with no model risk; the reviewer verifies the (obviously genuine) school letter by hand and the pathway verdict may legitimately stay red.
**Trade-offs:** A genuinely fake offer from a student who declared the STPM pathway also gets through the front door — the reviewer becomes the backstop instead of the gate. Acceptable because every case is human-reviewed. The exemption keys on the student-declared `chosen_pathway`, so a mis-declared pathway could dodge the offer-genuineness gate; the reviewer catches a pathway/offer mismatch (the red Pathway chip surfaces it).
**Revisit if:** the owner wants a genuine school-enrolment-letter recogniser (option a) so the pathway can go amber automatically, or wants presence dropped too for STPM.

## Sponsor pool: display vs fundable querysets for the funded grace window — Sprint 2026-07-21
**Decision:** A funded student lingers in the pool for POOL_FUNDED_GRACE_HOURS (default 48h) via a NEW `pool.display_pool_queryset` (`recommended` ∪ funded-within-window, keyed on `awarded_at`, states awarded/active/maintenance), used ONLY by the pool list + detail views. The existing `eligible_pool_queryset` (recommended-only) is untouched and still governs fundability, the public waiting-count, auto-sponsor allocation, and notifications. A `funded` serializer flag (status != 'recommended') drives the frontend "Funded" state.
**Alternatives considered:** (a) Widen `eligible_pool_queryset` itself to include funded students — rejected: four other call sites depend on its strict "still-waiting / still-fundable" meaning (count, standing_gift allocation, sponsor_notifications, an audit command); widening it would let a funded card be double-funded, inflate the public counter, and re-notify. (b) A scheduled job to hide funded cards after 2 days — rejected: unnecessary; a query-time window on `awarded_at` hides them for free with no cron, no writes, no state to reconcile. (c) A new `pool_hidden_at`/boolean column — rejected: same reason, no migration needed. (d) Key the window on the Sponsorship `offered_at` — viable but `awarded_at` is one field on the application (already annotated/selected) and marks the same moment; simpler.
**Rationale:** Owner wants social proof of momentum (a funded card with a full bar) without changing the money-safety invariants. Splitting display from fundable keeps every money/count/notification path strictly recommended-only while widening only the two read surfaces. The funding bar already rendered full off `funded_amount`, so the visual came for free.
**Trade-offs:** Two querysets to keep in step (both share the same anon-published + active-share-consent privacy gate; only the status/awarded_at clause differs). The grace window shows awarded/active/maintenance whose `awarded_at` is recent, so a fast-progressing student briefly appears as a "Funded" discovery card while already in-programme — acceptable (they WERE just funded). `closed` is excluded (a wrapped-up case, not a fresh win).
**Revisit if:** partial funding (TD-075) ships (the `funded` flag may need to distinguish partial vs full), or the owner wants the grace window to persist a "recently funded" marker independent of `awarded_at` (e.g. surviving a reopen).

## Sponsor pool: sort server-side, filter client-side — Sprint 2026-07-21
**Decision:** The pool's unfunded-first ORDER is applied server-side in `SponsorPoolListView` (Case annotation on status + Coalesce(awarded_at, recommended_at) for the within-group recency), while the All/Open/Sponsored FILTER is applied client-side in the students page over the already-fetched cards (on the `funded` flag), alongside the existing field/state/amount filters.
**Alternatives considered:** (a) Sort client-side too — rejected: "newest-first within group" needs awarded_at/recommended_at, and exposing those timestamps on the anonymised card would widen the allowlist (a privacy regression) for no benefit; keeping the sort in the query keeps the card timestamp-free. (b) Filter server-side (a query param) — rejected: the page already fetches the whole pool and filters field/state/amount on the client, so a status query param would split filtering across two layers for a small list; client-side is consistent and instant. (c) A visual separator / two sections for unfunded vs sponsored — rejected by the owner ("no physical separation, just sort order").
**Rationale:** Put each concern where its data lives. The sort depends on server-only timestamps → query. The filter is a pure view over fetched data → client, matching the existing filters. The display vs fundable queryset split (prior sprint) already isolates this from the waiting-count and fundability, so ordering the display set has no money/count side-effects.
**Trade-offs:** Sorting and filtering live in different layers (one Django, one React) — a reader must look in both places to see "how the pool list is shaped". Documented here and cross-referenced in both files' comments. Within-group recency relies on recommended_at/awarded_at being set (they are, at QC-accept and fund_student respectively).
**Revisit if:** the pool grows large enough to need server-side pagination (then filtering must move server-side too), or partial funding (TD-075) changes what "Sponsored" means for the filter.

## Sponsor portfolio: supported_semesters (nullable + heuristic) and one priority-ordered status — Sprint 2026-07-21
**Decision:** (a) `supported_semesters` is a NULLABLE field the owner fills per student; when NULL it falls back to the heuristic `award_amount // 1000` (RM1,000 ≈ one semester). (b) The sponsor sees ONE lifecycle badge per student (`pool.sponsor_portfolio_status`), resolved in priority order (discontinued > graduated > paused > semester_completed > needs_attention > on_track), replacing the previous composition of a separate support_status pill + progress badge.
**Alternatives considered:** (a-alt1) A required `supported_semesters` with a backfill — rejected: the true count varies by pathway AND offer letter (UA/Poly 5-6, PISMP 10) and the owner doesn't have all values yet; a nullable field with a correct-today heuristic ships now and lets them fill 100% over time. (a-alt2) Derive purely from pathway rules in code — rejected: encodes a fast-moving policy in code and still can't cover the offer-specific "varies" cases; the field is the honest home for those. (b-alt) Keep two badges (support flag + progress) — rejected: the owner's refined taxonomy makes the states mutually exclusive (a student is needs-attention OR paused OR discontinued, not several), so one priority-ordered badge is clearer and avoids the old overlap (e.g. "Graduated" showing twice).
**Rationale:** The award model already encodes RM1,000/semester (STPM 3000=3, continuing 1000=1, else 2000=2), so the heuristic is correct for every amount in use; the nullable override is the owner's path to 100%. One badge matches the owner's "these are separate states" intent and removes the duplicate-label confusion.
**Trade-offs:** The heuristic under-counts the not-yet-funded longer pathways (UA/Poly 5-6, PISMP 10 are funded at the flat RM2,000 = 2 today) — acceptable, and the field overrides it. The old `myStudents.progress.*`/`.support.*` i18n keys are now dead (left in place to avoid churn; harmless, parity kept). `portfolio_status` runs one extra count() query, but only for a funded card that reaches the semester check (a 'recommended' discovery card returns None before any query — no grid N+1).
**Revisit if:** the award model is expanded to actually fund the longer pathways (then supported_semesters should be set at award time), or partial funding (TD-075) changes what a "semester" of support means.

## Sponsor "my-students" detail: a separate owner-gated endpoint, not the discovery-pool detail — Sprint 2026-07-21
**Decision:** Added a NEW `GET /api/v1/sponsor/my-students/<pk>/` (`SponsorMyStudentDetailView`) that returns a student the CALLER sponsors, at any lifecycle status, gated to the caller's own Sponsorship (404 otherwise). It reuses `SponsorSponsorshipSerializer` + the reviewed anon profile. The existing discovery detail (`/sponsor/pool/<pk>/`) is unchanged.
**Alternatives considered:** (a) Reuse `/sponsor/pool/<pk>/` for the My-students detail — rejected: it's scoped to `display_pool_queryset` (recommended ∪ funded-within-grace), so it 404s a student the sponsor funded once the grace window passes — exactly the students the portfolio must show. (b) Widen the discovery detail's queryset to include a caller's funded students — rejected: it would blur "the anonymised discovery pool" with "your portfolio", complicate the funding/read gates, and risk leaking a funded student to a non-owner. (c) Serve the whole detail from the wallet payload the portal already loads — rejected: the wallet card lacks the anon PROFILE markdown, and a per-student page deserves its own fetch/deep-link.
**Rationale:** The two surfaces have different ownership models — the pool is "any approved sponsor sees anonymised candidates"; the portfolio is "you see the students YOU fund". A dedicated endpoint makes the ownership gate explicit (own sponsorship or 404), serves every status, and keeps the pool endpoint's grace-window semantics intact.
**Trade-offs:** A second detail endpoint + serializer to maintain, overlapping ~80% with the sponsorship serializer (mitigated by extending it). The page has no funding path by design — a sponsor wanting to fund another student uses the discovery pool, not this page.
**Revisit if:** partial funding (TD-075) lets multiple sponsors co-fund one student (the ownership gate then needs "any of my sponsorships"), or the spending panel (Sprint 3) needs data the sponsorship serializer doesn't carry.

## Vircle activation tracked only in the sheet's manual "Activated On" column — Sprint 2026-07-21
**Decision:** The 48h activation-request cron (`vircle_activation_request`) determines "installed but not activated" purely from the `Vircle_account` relay sheet: eWallet ID present (installed) AND the owner's MANUAL "Activated On" column (col I, outside the DB-synced A–H range) blank. Activation is never modelled in the app; the owner fills "Activated On" by hand when Vircle switches an account on, which prunes it from the request. The cron gains the first inbound Google-Sheet READ (`sheets.read_sheet_values`).
**Alternatives considered:** (a) Add a `vircle_activated_at` field + emit a generated "Activated On" column so activation is first-class and automated — rejected by the owner: **nothing reports activation back** (Vircle gives us no signal, and the first payment succeeding is too late/indirect), so there is no honest programmatic source; a manual column is the truth. (b) Infer activation from a successful first payment — rejected: indirect and lagging. (c) A dedicated "installed-but-not-activated" sheet the students fill — moot: the file at that path IS the auto-generated relay sheet, so the flow filters it instead.
**Rationale:** activation genuinely lives only in the owner's head/Vircle; encoding it in the DB would fake a signal we don't have. Reading the manual column keeps the truth where it actually is and needs no new model/migration.
**Trade-offs:** the cron depends on the owner keeping "Activated On" current (a stale blank keeps re-emailing Vircle; a wrongly-filled cell drops a student silently). The 15-min relay sync rewrites A–H, so anything the flow needs from the DB (name/nric/phone/ewallet) is safe, but the manual column must stay in col I (past H) or the sync would clobber it.
**Revisit if:** Vircle ever exposes an activation status we can read (then automate the column), or a real "installed date" is captured (today "Installed Date" ← "Confirmed on").

## Vircle activation email reference copy = Bcc + Drive archive (A+B) — Sprint 2026-07-21
**Decision:** Each sent activation email keeps a reference copy two ways: a **Bcc** to a mailbox (`VIRCLE_ACTIVATION_BCC`, else `ADMIN_NOTIFY_EMAIL`) and the **CSV filed to Drive** `01 BrightPath/03 Vircle/03 Activation` by the command after the send.
**Alternatives considered:** (a) The sender's actual Gmail "Sent" folder — rejected: we send via Brevo SMTP, not Gmail, so it would require adding a `gmail.send` scope to the SA's domain-wide delegation and inserting into Sent (heavier, new scope). (b) A DB model logging each send — rejected: heavier than needed; the Bcc + Drive CSV already give a human-readable record. (c) Bcc only — insufficient: the owner wanted a filed artifact too.
**Rationale:** Bcc + Drive reuse existing patterns (the payment email CCs + files its CSV to Drive), need no new scope or model, and give both an in-mailbox and an in-Drive reference.
**Trade-offs:** the Drive archive needs the `03 Activation` folder to exist (the filer never creates folders — a missing folder degrades gracefully: email still sends, archive skipped + logged).
**Revisit if:** the owner wants the email literally in the Gmail Sent folder (add the Gmail scope + API send).

## Org-admin reject is immediate + irreversible (no cool-off) — Sprint 2026-07-21
**Decision:** The org-admin reject of a stuck SHORTLISTED applicant (`services.org_admin_reject`, bucket `incomplete`) records the rejection AND sends the decline email in the same call — no `DECLINE_COOLOFF_DAYS` embargo, no `decline_due_at`, no Cancel banner. The owner asked for a 48h delay initially, then reversed it after I established that the *decision* was already immediate: the owner's actual concern was "prevent the student from submitting a document and changing status", and `_record_reject` achieves that on its own, because every student write path (`ApplicationDetailView.patch`, `IncomeRouteSwitchView`, `_current_application` for document uploads, `confirm_profile`) and `send_application_reminders` all gate on a status the case has left. Given the lockout was free, the owner chose to drop the delay entirely.
**Alternatives considered:** (a) The 48h email embargo as originally requested — rejected by the owner once the lockout was shown to be independent of it; it would also have inherited the embargo's student-status MASKING (`ApplicationReadSerializer.get_status` returns 'interviewed' during an embargo), which for a shortlisted student shows a stage they never reached while their uploads silently 403 — see TD-164. (This clause originally read "the FIRST live use of the masking"; corrected 2026-07-22 — `DECLINE_COOLOFF_DAYS` is **7** in production, not the code default 0, so the embargo path is already live and two applications were mid-embargo when this was written.) (b) Reuse `AdminRejectView`/`admin_reject('interview')` with `cooloff=timedelta(0)` — rejected: that endpoint's `_require_app_write` also admits a `qc` and the assigned reviewer, and the 'interview' decline copy ("thank you for COMPLETING your application… we were only able to support the students who most closely met our criteria") is false for a student who never completed.
**Rationale:** the delay bought a Cancel window but not the lockout, and the owner values finality here; the three-step confirm (button → mandatory reason → "Are you sure?") is the safety net instead.
**Trade-offs:** THERE IS NO UNDO. A mis-click that clears all three steps emails the student and closes the case; recovery is a manual DB correction. This is why `comments` is mandatory server-side (not just disabled-button UI) and why the confirm is in-page rather than a `window.confirm` a browser can suppress.
**Revisit if:** a mistaken rejection actually happens in production (then reinstate a short embargo + the existing Cancel banner, which still works — `cancel_pending_decline` is untouched).

## Reopened: a rejection reason DOES need its own field — Sprint 2026-07-21
**Decision:** Add `ScholarshipApplication.rejection_comments` (TextField, migration 0108), holding the org admin's reason verbatim. This REOPENS the 2026-07-19 decision "QC outright-reject reuses the reopen audit record (no new field/migration)", under that entry's own **"Revisit if: the owner wants a rejection reason surfaced independently of the reopen trail."**
**Alternatives considered:** (a) Keep hanging the reason on a `DecisionReopen` row as the QC reject does — rejected, and not merely undesirable but IMPOSSIBLE here: at `shortlisted` no verdict has ever been recorded, so there is no decision to reopen and `reopen_decision` has nothing to attach to. (b) Stuff the reason into `officer_verdict` (a JSON blob that already exists) — rejected: that blob is the reviewer's recorded verdict, and a case rejected at shortlisted has none; overloading it would put two unrelated meanings in one field. (c) Put the reason in the student's email — rejected by design: free-typed, single-language officer text must not reach a trilingual student mail, and the note may be candid ("suspected fabricated payslip").
**Rationale:** the earlier decision was correct for the QC path and stays in force there — this is the case its own Revisit clause anticipated, not a reversal.
**Trade-offs:** two reason-storage mechanisms now coexist (the reopen trail for QC declines, the field for org-admin rejects). Pinned by a test asserting the reason never appears in the outgoing email.
**Revisit if:** a third reject path appears — then unify all three on the field and render the reopen trail from it.

## Payment eligibility is judged on the COVERED MONTH, not the run's payment date — Sprint 2026-07-22
**Decision:** `payments._has_started` compares BOTH the pathway payment floor AND the student's `reporting_date` against `period_month` (the 1st of the month being paid for), never against `payment_date`. Owner's rules, locked 2026-07-22: (1) backpay is allowed — July may be paid on 15 Sep; (2) advance pay is allowed from the 25th of the previous month, and every student who qualifies for that month may be paid early; (3) a student qualifies for a month only if they reported STRICTLY BEFORE it began ("a student who reports on 1 July does not qualify for July pay"), from which the pathway floors follow (diploma report in July → first paid August; PISMP report in August → first paid September).
**Alternatives considered:** (a) Keep the payment-date comparison and instead require the run to be dated inside the covered month — rejected: it forbids both of the owner's rules outright, and the module already supports paying ahead (`_already_paid_for_period` is explicitly period-based). (b) Drop the pathway floor and derive everything from `reporting_date`, since the owner described the floors as a consequence of reporting patterns — rejected: 3 live rows have no reporting date at all, and the floor is the only gate protecting them; keeping both (AND) also lets the floor override an unusually early reporting date, which the owner's "all diploma students are only paid from Aug onwards" requires. (c) `reporting_date <= period_month` — rejected: the owner's worked example makes 1 July NOT qualify for July, so the comparison is strict.
**Rationale:** eligibility for a month is a property of the month; the payment date only governs whether the run may be *made* (a separate rule). Every other check in the module was already period-based — the pathway floor was the lone outlier.
**Trade-offs:** `payment_date` is now vestigial inside `eligibility()`, retained only as the fallback source of the period when `period_month` is omitted (removing it would churn ~15 positional call sites in tests for no behavioural gain). The `_has_started` name now reads slightly oddly for a month-based question.
**Revisit if:** the owner ever wants a student paid for a month they reported *into* (today: excluded), or the floors and reporting rule are unified.

## Advance runs are REFUSED before the 25th, not emitted empty — Sprint 2026-07-22
**Decision:** `create_run` raises `PaymentsError('too_early')` when `payment_date < earliest_payment_date(period_month)` (the 25th of the preceding month). The API returns `{code: 'too_early', earliest: '<date>'}` and the cockpit renders that date in the message.
**Alternatives considered:** (a) Create the run and let zero students qualify — the owner's literal phrasing ("no student should qualify"), but rejected WITH the owner's agreement: an empty draft explains nothing, leaves clutter, and invites "why is this list blank?". (b) Compute the earliest date in the frontend to render the message — rejected: it would make the rule a keep-in-sync pair across Python and TypeScript, the exact failure mode this repo has hit ~5 times (verdict tally↔chip paint, `_SUBJECT_BM`↔`subjects.ts`, assignment offer-set↔accept-set). Returning the computed value keeps one source of truth. (c) A separate "earliest date" endpoint for the form to pre-validate — deferred: more surface for a rule the error already communicates at the moment it matters.
**Rationale:** one condition expresses both owner rules with no special case for backpay (a past period's 25th-of-previous is long gone, so any run passing the `past_date` check satisfies it), and it correctly refuses runs two or more months ahead.
**Trade-offs:** the guard is on the payment date, so an admin can still *prepare* a run any time (owner's case 3) — the form gives no warning until submit. `adminMutate` now retains the whole error body (`err.body`), a small widening of the client error shape.
**Revisit if:** the owner wants the 25th to become configurable per organisation, or wants the date picker to block invalid dates before submit.

## The reporting date is a first-class fact with one owner — Sprint 2026-07-23
**Decision:** `services.sync_reporting_date_from_offer()` is the single writer of the offer-derived reporting date, called at the TOP of `autofill_pathway_from_offer` (before its four pathway guards) rather than at the bottom. The continuing-vs-fresh question is answered by `pathway_engine.course_start_year()` / `started_before_cohort()`, which read the offer's INTAKE YEAR first and the stored date second, and are shared by BOTH consumers (`award._stpm_continuing`, `income_engine.semester_result_gap`).
**Alternatives considered:** (a) Keep the write inside the autofill and just move it above the guards — rejected: it stays a passenger in a function about the pathway, so the next guard added below it re-creates the bug (which is exactly what happened after the LOCK guard was fixed and tested in an earlier round). (b) Populate the date by parsing the student's free-text answer to the `reporting_date_unknown` clarify, reusing the Vircle-ID/pension-claim pattern — REJECTED after checking live data: 1 of the 2 parseable answers was wrong (#123 gave the date he collected a confirmation letter), and it would have sized his bursary at RM3,000. (c) Derive a reporting DATE from the intake month (e.g. `2025-06-01`) so the column is never NULL — rejected: fabricated day-precision in a field the payment gate reads; the award rule needs only the YEAR, so it reads the year directly and the column stays honestly empty.
**Rationale:** three rules depended on one field; the field was not the question (a Form 6 letter carries an intake range and no date at all), and the copy was inheriting the exits of an unrelated function.
**Trade-offs:** `course_start_year` runs `student_offer_check` on the latest offer, so the award rule now does document work it previously avoided — acceptable (it is already called throughout the cockpit and the verdict path). A student whose letter genuinely lacks a date still ends with a NULL column until an officer records it.
**Revisit if:** the extractor learns to read intake ranges into `reporting_date` directly, or a pathway other than STPM starts pricing on the start year.

## QC cannot accept without a reporting date — an ABSOLUTE stop — Sprint 2026-07-23
**Decision:** `AdminQcDecisionView` refuses `accept` when `reporting_date` is NULL (`reporting_date_required`), with NO override — unlike the red-fact gap floor, which a super or qc may pass by recording a reason. The officer clears it via `AdminReportingDateView`, surfaced as a box directly above the Recommendation card, shown only when the offer letter carries no readable date AND the case is in the reviewer's window (`interviewing`, or a decision that has been reopened).
**Alternatives considered:** (a) Overridable with a recorded reason, matching the existing floor — my recommendation, REJECTED by the owner: the honest remedy is to record the date, not to wave the case through, and the gate is cheap because it almost never fires (after the intake-year fix, every current STPM student resolves without it). (b) Let QC fill the date in place — rejected: the reviewer owns the case's facts, so QC reopens and it goes back, which is also why the box is hidden at `interviewed`. (c) Block the reviewer's submit as well — rejected as over-constraining: the box sits above Recommendation so the reviewer meets it while deciding, and QC is the backstop for the ones that slip through.
**Rationale:** an absolute stop is only a trap if there is no way to clear it; pairing it with the entry box (which did not exist — `reporting_date` was read-only across the whole admin surface) makes it a gate instead.
**Trade-offs:** a case whose date cannot be established at all cannot be accepted, by design. A reopen is required to clear one that reaches QC empty — one extra round trip, accepted by the owner.
**Revisit if:** the stop starts firing often enough to be an obstacle, which would mean the extractor has regressed.

## No provenance columns for a hand-entered reporting date — Sprint 2026-07-23
**Decision:** An officer-entered date is stored in `reporting_date` with no `source`/`by`/`at` columns. Attribution lives in the `AUDIT reporting_date_set` log line; the cockpit distinguishes a typed date from a documented one because its verified tick is derived from DOCUMENT corroboration (`lib/fieldVerification`), so a hand-typed value renders without one and never borrows "Matches the offer letter". A later readable letter deliberately OVERRIDES an officer's value.
**Alternatives considered:** (a) The three-column trio matching the house pattern (`rejected_by`/`rejected_at`, `qc_override_reason`/`_by`/`_at`) — built, then REMOVED at the owner's direction as over-engineering for a rare one-off. Worth recording that the owner was right for a reason I had missed: I added the columns to let the UI distinguish typed from documented, and the UI already derives that independently — the schema would have duplicated a signal that was free. (b) A single `source` column — rejected for the same reason.
**Trade-offs:** who entered a date is queryable only in logs, not in SQL. If hand-entry ever becomes common rather than exceptional, revisit.
**Revisit if:** officer-entered dates stop being rare, or a report needs to segment them.

## A pathway switch is detected by PROGRAMME, not institution — Sprint 2026-07-23
**Decision:** `promotion.is_pathway_switch()` compares the extracted **programme** on two offer letters; a difference bypasses the quality comparison in `should_promote` so the newer letter takes the live slot even when it reads worse. `usable` remains a floor.
**Alternatives considered:** (a) Compare the INSTITUTION (the owner's first proposal: same institution → quality, different → date) — rejected on the data: #107 uploaded ONE PISMP letter twice and the institution extracted as "INSTITUT PENDIDIKAN GURU MALAYSIA" on one pass and "INSTITUT PENDIDIKAN GURU KAMPUS KOTA BHARU" on the other (the letterhead body vs the campus, same page), so an institution test reads a re-scan as a switch. Its programme string was identical on both passes, and #13's genuine switch differs clearly on programme ("FTV - ASASI TEKNOLOGI KEJURUTERAAN" vs "DSPD - DIPLOMA SAINS KOMPUTER"). (b) Rank offer letters by quality like income proofs — rejected: it would have kept #13's OLD letter and buried the course he actually moved to. (c) Newest always wins — rejected: it removes the protection against a worse re-scan burying a good letter, which is what the promote gate is for.
**Rationale:** an offer letter is the one key document where a second copy may be a NEW FACT rather than a better copy of the same fact; the programme is the field that distinguishes them, and it is the field the pathway engine already keys on.
**Trade-offs:** a switch WITHIN the same programme (same course, different institution) reads as a re-scan and is still subject to the quality bar — acceptable, since the pathway itself is unchanged. A blank/unread programme on either side falls back to the quality comparison, deliberately.
**Revisit if:** a real case appears where a student switches institution but keeps the identical programme string and the new letter reads worse.
## Income is satisfied by EVIDENCE, not by the declared route — Sprint 2026-07-23
**Decision:** `income_engine.salary_income_satisfied` no longer opens with `if income_route != 'salary': return False`. It asks whether ANY working member has a complete, clean cluster, resolving earners through a new opt-in `effective_working_members(app, any_route=True)`. `income_doc_blockers`'s STR branch gains the salary branch's existing early-return, so a student is not asked for the STR triplet when a documented earner already settles income. Owner rule: *"it is either STR or salary — students may fulfil both but they are not required to."*
**Alternatives considered:** (a) Widen `effective_working_members` itself — REJECTED: it is also called by `verdict_engine` and `profile_engine`, so the default had to stay untouched or the gate fix would have moved verdicts, bands and generated profiles. The opt-in flag keeps the blast radius inside the submission gate. (b) Special-case #116 in the STR branch — rejected as a per-case patch over a policy bug. (c) Auto-switch the student's route when the STR fails — rejected: the route is the student's declaration, and rewriting it would make the cockpit disagree with what they answered.
**Rationale:** `document_red_blockers` ALREADY skipped every income-cluster red (including `str`) once `income_established` was true, and `income_established`'s docstring already claimed to work "on EITHER route". The policy was built; a single route check disabled half of it. #116 was trapped by a failed STR while her mother was fully documented — IC, a genuine payslip, and a birth certificate linking them — and that evidence was never even inspected.
**Trade-offs:** a student who declared STR and uploaded no STR at all now passes on salary evidence alone. That is the owner's stated policy, not a side effect. Guard-rail: with no complete cluster a failed STR still blocks (locked by test), so the relaxation cannot wave through an application with no income evidence.
**Revisit if:** the two routes stop being interchangeable, or a third income route is added.

## Per-org branding — frontend delivery is `NEXT_PUBLIC_ORG_CODE` + a public endpoint, dark for platform — Sprint 6, 2026-07-24
**Decision:** A build-time env var `NEXT_PUBLIC_ORG_CODE` decides the frontend's branding source. Unset (or `'brightpath'`) ⇒ platform mode: the app renders baked `PLATFORM` defaults from `src/lib/branding.ts` and **never fetches** anything. Any other value triggers exactly one fetch, at `BrandingProvider` mount, to the new `GET /api/v1/branding/<slug:code>/`.
**Alternatives considered:** (a) Always fetch, even for the platform tenant — rejected: adds a network round-trip and a loading-flash risk to every BrightPath page load for zero behavioural gain today. (b) Bake branding into the build per-tenant (a build-time config file instead of a runtime fetch) — rejected: would require a separate deploy artifact per tenant, defeating the "one app, many tenants" goal the platform roadmap is building toward.
**Rationale:** BrightPath is the only live tenant and must render byte-identically with zero added latency or flash; a future tenant's cost (one fetch) is paid only by that tenant, never by BrightPath.
**Trade-offs:** a future tenant's very first paint either blocks briefly on the fetch or shows a platform-default flash before the real branding arrives — acceptable for v1, revisit if a second tenant's onboarding experience needs to be flash-free.
**Revisit if:** a second real tenant's UX requires zero-flash branding (would need SSR/edge-config delivery instead of a client fetch).

## `t()` auto-injects branding tokens rather than threading them per call site — Sprint 6, 2026-07-24
**Decision:** `t()` (the custom i18n function) merges 5 branding params (`programmeName`, `orgShortName`, `personaName`, `supportEmail`, `displayDomain`) beneath any explicit params the call site passes, for every call, rather than requiring each of the ~15 call sites that need a brand token to import and pass `useBranding()` output explicitly.
**Alternatives considered:** Per-call threading — rejected. With ~18 message keys across ~15 call sites needing a brand token today (and more expected as tenancy work continues), each site is a chance to forget the import and render a literal `{programmeName}` placeholder to a real student. Auto-injection is one audited change instead of 15 unaudited ones.
**Rationale:** makes brand-token safety free for every future string, not just today's 18 — the tokenisation rule (Rule 2 in the sprint's conventions) becomes structurally hard to violate rather than a reviewer checklist item.
**Trade-offs:** every `t()` call now does a small amount of extra merge work (mitigated by the `'{'`-absent fast path, which skips interpolation entirely for the majority of calls that have no placeholder at all); a call site CAN still override any of the 5 tokens explicitly if it ever needs to (explicit always wins).
**Revisit if:** the branding token set grows large enough that the per-call merge cost becomes measurable, or a call site needs per-instance branding different from the page's ambient `BrandingProvider` context.

## Theme CSS variables use the RGB-channel form, not hex — Sprint 6, 2026-07-24
**Decision:** The ten `--brand-N` CSS custom properties store space-separated RGB channel triples (e.g. `19 127 236`), consumed via Tailwind's `rgb(var(--brand-500) / <alpha-value>)` pattern, rather than storing a hex string directly.
**Alternatives considered:** Hex custom properties (`--brand-500: #137fec`) consumed via `var(--brand-500)` directly — rejected outright once verified against the actual codebase: the existing Tailwind config already uses OPACITY MODIFIERS (`bg-brand-500/10`, `/40`, `/20`) at several live sites (the admin payments table, profile tiles), and Tailwind's opacity-modifier syntax only resolves against the `rgb(... / <alpha-value>)` pattern — a bare hex custom property cannot support it.
**Rationale:** preserves every existing opacity-modifier usage without hunting down and rewriting each call site; verified by spot-checking the compiled CSS output at the two known opacity-dependent sites before treating the theme phase as done.
**Trade-offs:** the RGB-channel string is less human-readable at a glance than a hex value in devtools; a comment in `globals.css` documents the conversion source (today's exact hex ramp).
**Revisit if:** a design-system rework drops all opacity-modifier usage (unlikely; not worth pre-emptively simplifying).

## Bare-brand message sites use `{orgShortName}`, not `{programmeName}` — Sprint 6, 2026-07-24
**Decision:** Two message keys (`sponsorPool.verifiedByBrightPath`, `sponsorPortal.students.balanceLabel`) that read a BARE brand mention ("Verified by BrightPath", a balance label) interpolate `{orgShortName}` (e.g. "BrightPath") rather than `{programmeName}` (e.g. "BrightPath Bursary") — the token used everywhere else in this sprint.
**Alternatives considered:** Use `{programmeName}` everywhere for consistency — rejected: at these two specific sites the full programme name reads oddly ("Verified by BrightPath Bursary", "BrightPath Bursary balance") where the short organisation name is what a reader expects.
**Rationale:** the token choice follows what a human would naturally write at that exact sentence position, not a blanket rule; `org_short_name` was added to `branding.py` as its own accessor (with a platform default of the literal `'BrightPath'`, since there is no dedicated database column for it — see TD-170's related note) specifically to serve this split.
**Trade-offs:** two different brand tokens now exist in the message files, which a future contributor must understand the distinction between; documented in the brief and this decision, not left implicit.
**Revisit if:** a tenant's short name and full programme name diverge enough that this split needs to become a general convention rather than a two-site exception.

## Consent + legal text is interpolated, gated on a byte-identity snapshot — Sprint 6, 2026-07-24
**Decision:** The consent text (`scholarship.consent.text`/`textMinor`) and the legal pages (terms/privacy) DO get the `{programmeName}` token, rather than being left as permanent BrightPath-only literals — but only after a snapshot test froze the fully-rendered en/ms/ta strings BEFORE any edit, so the edit is provably byte-identical for BrightPath.
**Alternatives considered:** Leave consent/legal text un-tokenised as an explicit tenancy exception (some projects treat legally-reviewed text as immutable-per-tenant, requiring a full re-review for every org) — rejected for this sprint per owner instruction: the interpolation is approved AS LONG AS BrightPath's rendered output doesn't change, with the owner reviewing the final templated wording before any future tenant's onboarding, not before this deploy (since nothing changes for BrightPath).
**Rationale:** keeps the legal-text maintenance burden at one canonical template instead of a fork per tenant, while the byte-identity snapshot removes the risk that this sprint itself silently altered wording a lawyer already vetted.
**Trade-offs:** a future tenant's consent/legal text is only as good as the interpolation point chosen today; if a tenant's context requires materially different legal language (not just a different name), the template approach breaks down and needs a real per-tenant legal review.
**Revisit if:** a second tenant's legal counsel requires wording beyond a name substitution.

## TD-169 stays parked; the "BrightPath Foundation" tenant name and support phone stay literal — Sprint 6, 2026-07-24
**Decision:** Three items were explicitly left OUT of this sprint's de-hard-coding scope: (1) TD-169 (the Vircle guide PDF attachment filename, logged at Sprint 5's close) is not resolved here — it needs a tenant-aware guide asset, not a frontend change. (2) `admin.administration.tenantName` ("BrightPath Foundation") is deferred as tenant content and added to the brand-guard's documented allowlist rather than tokenised. (3) The support phone number stays a literal (unlike the support EMAIL, which was tokenised).
**Alternatives considered:** Fold all three into this sprint's scope for completeness — rejected: (1) is genuinely backend/asset work, not frontend; (2) "BrightPath Foundation" is a specific legal-entity name, not a generic brand mention, and conflating the two risks a tenant's onboarding accidentally implying the wrong legal entity; (3) no tenant-configurable phone field exists yet, and adding one wasn't in the brief's verified-facts scope.
**Rationale:** each is either out of this sprint's layer (frontend) or would require new backend schema/fields not yet designed — bundling them in risks scope creep on a sprint that was deliberately kept to "de-hard-code what's already columnar or trivially derivable".
**Trade-offs:** a future tenant onboarding will need explicit follow-up work for all three; documented here and in the brand-guard's allowlist comments so nobody mistakes the omission for an oversight.
**Revisit if:** a second tenant's onboarding actually needs any of the three (a new support-phone column, a tenant-aware Vircle guide, or a non-"BrightPath Foundation" legal entity name in the admin UI).

## The `info@` mailto is built from `frontendDomain`, not a dedicated seam field — Sprint 6, 2026-07-24
**Decision:** The `info@` mailto links (`scholarship/page.tsx` ×2) are constructed as `info@${frontendDomain}` at the call site, reading the existing `displayDomain` branding token, rather than adding a dedicated `email_info`/`email_general` field to the branding seam.
**Alternatives considered:** Add a dedicated seam field for the info-alias, mirroring how `supportEmail`/`sponsor@` are handled — rejected for this sprint: the `info@` convention (info@ + the tenant's own domain) held for every case examined and needed no per-tenant override, so a dedicated field would be an unused extension point.
**Rationale:** minimises the seam's surface area to fields that are actually exercised; `displayDomain` already existed for other purposes, so this is free.
**Trade-offs:** if a future tenant's info-alias convention ever diverges from `info@{domain}` (e.g. a tenant wants a different local part), this becomes a small follow-up rather than a zero-cost config change.
**Revisit if:** a tenant needs an info-alias that isn't `info@` + their domain.

## The Blockers card renders the STUDENT'S gate, never a second computation — Sprint 2026-07-23
**Decision:** The officer's Blockers card is driven by `application.consent_blockers` — the exact list the student's own consent POST enforces — rather than by a cockpit-side calculation. `lib/blockers.ts` only maps codes to copy and works out which wizard step to send them to; it computes no eligibility of its own.
**Alternatives considered:** (a) Derive the card from the 8-part `completeness` booleans that fed the old banner — rejected: that is exactly why the banner could only ever say "Required documents / Consent" with no detail. (b) Compute a richer officer-side list in the frontend — rejected outright: two implementations of "what does this student owe" will drift, and the officer would then tell the student something the student's own screen contradicts.
**Rationale:** the field was already on the admin payload and had never been rendered; its own docstring said it existed so the cockpit could answer *"why can't this student submit yet?"*. Zero backend work was required — the answer was already arriving and being thrown away.
**Trade-offs:** the card inherits the gate's vocabulary, so copy for 35 codes had to be written in the officer's voice; and the gate never emits a "consent missing" code (it IS the gate to consent), so that state is handled separately in the card.
**Revisit if:** officers need blockers that are not submission blockers (e.g. soft Check-2 items) — those belong in a different surface, not this list.

## Consent wording narrowed to what we actually do — draft-6, no re-consent — Sprint 2026-07-23
**Decision:** `scholarship.consent.text` / `.textMinor` (en/ms/ta) now state that sponsors receive an **anonymised summary**, are not shown name, NRIC, photograph, address or contact details (nor the parents'), and that **documents are never shared**. `CONSENT_VERSION` 2026-draft-5 → **2026-draft-6**. Consents already given are NOT re-collected.
**Alternatives considered:** (a) Leave it — rejected: the text promised away more than the platform does, and the public landing FAQ already described the truth, so the legally operative text was the only wrong copy. (b) Say "anonymous" — REJECTED as itself inaccurate: sponsors do see the secondary school, course and institution (decision 2026-07-18), so the consent says "anonymised summary" and then lists what they DO see. (c) Re-collect consent from the 43 awarded students — rejected: the new text is NARROWER, so they permitted more than we do; narrowing a promise is not a breach. (d) Render each record's text at the version agreed — deferred, see trade-offs.
**Rationale:** a consent that overstates is a compliance risk and needlessly alarming to families, and it contradicted our own marketing copy.
**Trade-offs:** the "What you agreed to" panel renders the CURRENT wording, so past consenters now see the corrected, narrower text rather than the words they actually agreed to. Accepted deliberately (it reflects reality and is narrower in their favour); logged as tech debt rather than built.
**Revisit if:** the sponsor payload ever widens — the allowlist is what makes this text true, so any new sponsor-visible field must be checked against this wording.

## STR recognition deliberately NOT broadened — payment receipts refused — Sprint 2026-07-23
**Decision:** The STR format gate stays as it is. A SARA-only page remains `unknown` → `wrong_type` (SARA is a different programme and ranks below STR), and a bank STR **payment receipt** is NOT accepted as proof. A scoped proposal to add a `payment_record` surface and capture Maklumat-Pembayaran credit dates was dropped.
**Alternatives considered:** (a) Accept a dated payment credit as proof of approval, on the logic that you cannot be paid without being approved — I proposed this; the owner REFUSED and was right: *"We don't know if it is a typed doc."* A MySTR portal screenshot comes from a verifiable government surface; a receipt could be fabricated, so accepting it would open a forgery path for marginal gain. (b) Treat `unknown` as blank (→ amber `unreadable`, non-blocking) so a misclassification never hard-blocks — not pursued once the income-gate fix removed the trapping behaviour.
**Rationale:** owner: *"the present system seems to work"* — and it does, because the genuine cases carry on the OTHER paths: #9 was awarded on officer judgement, #116 now clears on verified salary evidence, which is a better basis than an unverifiable receipt.
**Trade-offs:** genuine STR holders whose only proof is a receipt or a SARA page will still read amber/red and need officer judgement or the salary route. Accepted.
**Revisit if:** a verifiable, machine-checkable payment surface appears (e.g. a reference that can be validated against a government source), which would remove the forgery objection.
**Correction recorded:** I earlier called #9 a misclassification. It is not — the document genuinely is a SARA page and the parser read it correctly; I had inferred from a truncated `doc_seen` string without reading it. Only #115 was a true misclassification.

## Sources "Students" counts APPLICATIONS by referral chip, house org takes the residual — Sprint 2026-07-23
**Decision:** `_source_application_counts` (replacing `_source_student_counts`) counts `ScholarshipApplication`s attributed by the applicant's raw referral chip (`profile.referral_source`) — the same signal the Applications-list Source filter uses. Each external partner counts apps whose chip equals its `code`; **BrightPath** (the house org) takes the RESIDUAL: every application not claimed by a partner.
**Alternatives considered:** (a) Keep counting `StudentProfile.referred_by_org` — rejected: it counted the legacy course-selector registry (hundreds of non-applicant profiles, inflating CUMIG to 256) through an FK that had drifted. (b) Count applications by the FK instead of the chip — rejected: the FK still misattributes (2 self-referred apps pointed at CUMIG), and it leaves the house org at 0 because self-referred students have no FK at all.
**Rationale:** the column is read as "how many bursary applicants came through this partner"; it was answering a different question about a different population. Chip-based counting makes it agree with the filtered list an officer sees.
**Trade-offs:** the tally is global (single tenant today) and carries an `# org-fence:` pragma; the residual maths needs the total, so it must span all orgs.
**Revisit if:** a second house/tenant organisation is introduced.

## Cards with no possible use at a stage are HIDDEN, not disabled — Sprint 2026-07-23
**Decision:** At `shortlisted`, five cockpit cards are hidden behind one tested predicate (`officerCockpit.isPreSubmissionStage` / `showsPostSubmissionCards`): Recommendation, Rate AI Prediction, Interview Stage, Estimated need, Final profile.
**Alternatives considered:** (a) Disable them instead — rejected: a disabled card still consumes the screen and invites clicking. (b) Five inline `status !== 'shortlisted'` conditions — rejected: the rule would drift; it now lives in one place with tests, and `profile_complete` can be retired later by deleting one entry.
**Rationale:** a `shortlisted` application is pre-submission BY DEFINITION — submitting Step 2 is what moves it out — so `isDecisionReady` (which needs a submitted interview) can never be true there. Across 48 shortlisted applications, none had submitted Step 2. The Assignment card had carried exactly this guard for months; the interview SCHEDULER was already gated correctly and only its capture box had been missed.
**Trade-offs:** an officer who wants to pre-fill an interview form before submission no longer can — intended, since that form could not be saved anyway.
**Revisit if:** the lifecycle changes so that a shortlisted application can hold a submitted interview.

## Requests space v1 — hours-only quotes, no rate card yet — Sprint 15, 2026-07-24
**Decision:** `OrgRequest` quotes are denominated purely in hours (`quote_hours`, `quote_margin_pct`) — "≈N hours, includes M% margin" — with no ringgit figure anywhere in v1.
**Alternatives considered:** Ship the roadmap's original published rate card (bugs free, features priced in RM off a fixed table) as planned — rejected once scoping confirmed no hourly rate exists yet to price against; inventing one to hit the roadmap's original shape would have meant fabricating a number the business hasn't set.
**Rationale:** a quote must reflect a real commitment; hours are the fact the owner can actually commit to today, and the rate card becomes real data later once enough quoted work accumulates to derive a defensible rate.
**Trade-offs:** an org receiving a quote today can't see a cost, only an effort estimate — acceptable for v1 since no priced commercial relationship exists yet for any org.
**Revisit if:** an hourly rate is set for any org — at that point quotes should convert hours × rate into an actual RM figure, which is the "v2 rate card" the roadmap originally scoped.

## Requests space v1 — the 8th status, `deferred` — Sprint 15, 2026-07-24
**Decision:** The roadmap's 7-status flow (`submitted → triaged → quoted → approved → scheduled → done/declined`) gains an 8th status, `deferred`, reachable from `quoted` and re-quotable back to `quoted`.
**Alternatives considered:** (a) Force a requestee to either accept or decline a quote — rejected: a genuine "not now, maybe later" response would otherwise have to route through decline (which reads as a rejection) and a fresh re-submission (which loses the AI draft, triage note and clarification history). (b) Model deferral as a flag on `quoted` rather than a distinct status — rejected: the transition matrix (`org_requests.TRANSITIONS`) is the single source of truth for what actions are valid from where, and a flag would require every consumer of status to also check the flag, reintroducing exactly the two-source drift the matrix exists to prevent.
**Rationale:** a requestee's real options after seeing a quote are accept / modify the ask / not now / decline outright — the roadmap's flow only modelled three of those four.
**Trade-offs:** one more status for every future status-aware surface (i18n, tone maps, the transition matrix) to account for — accepted as a small, one-time cost.
**Revisit if:** deferred requests pile up unresolved and need their own SLA/reminder treatment (not built in v1).

## Requests space v1 — AI clarifying questions flow free; only the quote is owner-gated — Sprint 15, 2026-07-24
**Decision:** The AI reviewer's clarifying questions (0–3 per run) are appended to the request's `clarifications` thread and emailed to the requestee immediately, with no owner approval step. The AI's hours estimate and classification (`ai_draft_*`) remain strictly owner-gated — never visible to the org until the owner turns them into an actual triage + quote.
**Alternatives considered:** Gate every AI output behind owner approval, per the roadmap's literal "an AI verdict is always a DRAFT" wording — rejected by the owner as too slow for the low-stakes half of the AI's job: a clarifying question carries no commercial commitment, so gating it adds a manual approval step for no risk reduction.
**Rationale:** the roadmap's draft-only rule exists to stop an unapproved PRICE reaching an org, not to gate every AI utterance; splitting "ask" (free) from "quote" (gated) preserves the actual risk boundary while letting the AI move the conversation forward without waiting on the owner.
**Trade-offs:** an org could see a poorly-phrased or off-target AI question with no human review first — mitigated by the org fence + a hard cap of 3 auto-runs per request, and the owner can still manually re-trigger triage at any point.
**Revisit if:** a clarifying question ever leaks something quote-adjacent (e.g. an implied cost) — would need the free/gated split narrowed.

## Requests space v1 — optional Bugzilla-style scoping fields (component/urgency/steps-to-reproduce) — Sprint 15, 2026-07-24 (increment)
**Decision:** `OrgRequest` gained three OPTIONAL fields, added same-day as an owner-approved increment on top of the core sprint: `component` (10 choices derived from the admin nav's real modules), `urgency` (blocking/important/nice_to_have), and `steps_to_reproduce` (free text, bug-only). All blank-default; omission is never a 400.
**Alternatives considered:** Ship these as REQUIRED fields to force better-scoped requests from day one — rejected by the owner: v1's goal is to lower the barrier to submitting a request at all, and a longer required form works against that; optional scoping lets a requester add detail when they have it without blocking a quick submission.
**Rationale:** modelled on the fields a Bugzilla-style tracker asks for, scoped down to what this system can actually act on (there is no severity/priority triage automation yet, so `urgency` is informational, not a queue-sorter).
**Trade-offs:** the org-payload allowlist snapshot widened from 16 to 19 keys — a deliberate, reviewed change (not silent), caught by the exact-key-set snapshot test that exists precisely to force this kind of change to be visible.
**Revisit if:** `urgency` needs to actually drive triage ordering (today it is display-only).

## Requests space v1 — screenshots deferred; quote email to the submitting org_admin only; withdraw until quoted — Sprint 15, 2026-07-24
**Decision:** Three smaller operational rules locked for v1: (1) a request carries no screenshot/attachment (TD-172) — text description only; (2) the quote email goes to the org_admin who submitted the request, not broadcast to the whole org; (3) a requestee may withdraw a request any time before it is quoted (submitted/triaged/quoted states), and may still decline from `deferred`, but cannot withdraw once a quote exists (that path is decline, which requires a reason).
**Alternatives considered:** (1) Build attachments now, reusing the applicant-document vault (`ApplicantDocument`/`storage.py`) — rejected: that vault is shaped for a per-applicant signed-URL access pattern, not a general org-fenced attachment store, and bolting requests onto it risked either a cross-org leak or duplicating the pattern ad hoc rather than designing it properly. (2) Broadcast the quote email to every org_admin in the org — rejected: mirrors the "who acted" precedent used elsewhere (e.g. payments sign-off notifications) rather than a broadcast, and keeps the thread's accountability with the person who submitted it. (3) Allow withdraw at any status including post-quote — rejected: once the owner has invested triage + quote effort, a silent withdrawal loses that effort with no record; decline (with a required reason) is the appropriate path past that point.
...
## One gift per Programme — the Programme IS the gift, not a container for one — 2026-07-26
**Decision:** A Programme has exactly ONE gift offering. The Programme **is** the gift ("the BrightPath Bursary", "the Sabah Bursary") — it holds the rules, the amounts and the fund, and it is the DURABLE level: it never lapses, only its intake Years cycle. There is no Award level between Programme and Year. The word "award" is reserved for the LEAF — one student's individual grant (application → award → payment).
**Alternatives considered:** Insert an Award level between Programme and Year, so one funded project could run several distinct gifts (e.g. a bursary AND a laptop grant, or full vs partial awards) with different rules and agreements — rejected by the owner: every case in view (BrightPath flagship, BrightPath Sabah, Inspire) has exactly one offering per programme, and a level that distinguishes nothing must still be carried by every query, every fence check and every config screen.
**Rationale:** the owner's original phrasing put "the award" ABOVE the year, on the correct observation that *"BrightPath itself won't lapse, but newer students will be joining annually into the same gift programme every year."* That durability requirement is fully satisfied by the Programme being the durable level — the apparent disagreement about ordering came entirely from "award" being used for two different things (the offering vs one student's grant). Naming them apart resolves it without adding structure.
**Consequence for the build:** "create a programme and the bursary" is ONE durable object (the Programme), followed by opening an intake Year beneath it — **do not build two creation steps for the gift itself.**
**Trade-offs:** if a programme ever genuinely needs two distinct gifts, an Award level must be retrofitted after money is attached to programmes — the expensive direction. Accepted on the owner's judgement that this will not arise.
**Revisit if:** one funded project needs two distinct gifts with different rules, amounts and agreement templates. Retrofitting would mean re-homing the fund and the rules from Programme down to Award; the Year and the fence would be unaffected.

## Benefactor anonymity is absolute — a funder never learns a student's identity — platform invariant — 2026-07-26
**Decision (owner, verbatim):** *"Benefactors will not know the students. Period. Benefactor donates to the organisation, and selects the students based on their needs. They will never learn their identity."* This is a **platform invariant**, not a per-organisation setting and not a flag: no tenant may switch it off, and no feature may be built that erodes it. The money flows **benefactor → organisation → (allocated by the benefactor) → an anonymised student**; the benefactor chooses on NEED, never on identity.
**What this forecloses, permanently:** no "meet your student" reveal, no named thank-you letters or progress reports back to the funder, no student photographs, no de-anonymisation at any stage of the lifecycle — not at offer, not at award, not at graduation, and not with the student's consent (the student cannot waive an invariant that exists to protect the class of applicants, not just themselves).
**Alternatives considered:** the common bursary-sector pattern of revealing the student to their funder after award (thank-you letter, progress reports, sponsor-student correspondence) — rejected outright by the owner. Also considered and rejected: making anonymity a per-tenant configuration so a future organisation could run a named-sponsorship programme.
**Rationale:** this codifies what the build already assumes. `pool.py` states the boundary — *"a sponsor never sees a name/NRIC/address/phone/email/school/photo"* — students appear only as a salted-hash alias (`pool_ref` → `S-A3F9C1`, deliberately non-sequential so pool size and application order don't leak), and `Sponsorship` already holds anonymity BOTH ways (the student never learns the funder's identity either). Writing it down as an invariant means a future sprint cannot erode it by increments.
**Design consequence — the card must carry NEED, not identity.** "Selects the students based on their needs" only works if the anonymised card conveys enough genuine need signal to choose on. Any programme-scoping work (P3) filters WHICH cards a funder sees; it must never touch the allowlist that governs WHAT a card shows. The existing anonymity assertions must pass unmodified through P2/P3/P4.
**Closes P4 open question 2:** the benefactor **self-serves** allocation — browsing the anonymised pool and funding students from their programme-scoped wallet, i.e. the existing sponsor flow narrowed to one programme. The org admin does NOT allocate on their behalf, so no additional admin scope is needed. (Consistent with the existing `Donation` semantics: a donation is final and can never be withdrawn to a bank — only redirected within the platform.)
**Trade-offs:** forgoes the donor-relations value of a named connection, which some funders expect and which can drive repeat giving. Accepted deliberately, and it is the safer default for a B40 applicant pool.
**Revisit if:** never, on the owner's instruction. A future request for named sponsorship is a different product, not a configuration of this one.

## Need stays PROSE on the sponsor card — no need band, no need score — 2026-07-26
**Decision:** The anonymised sponsor card conveys need through the ≤20-word `blurb` (prose, and deliberately somewhat subjective in tone) — NOT through a banded or scored field. No `need_band`, no per-capita bracket, no B40 tier on the card. This stands even though the card already carries a banded `academic_band`, and even though "the benefactor selects on need" puts weight on this signal.
**Alternatives considered:** add a coarse, non-identifying need band mirroring the existing `academic_band` pattern (e.g. a per-capita or household-income bracket) — proposed by the agent on the grounds that the card is currently rich on merit signal (academic band, course, target institution) and thin on need, so a funder choosing "on need" has little to weigh. **Rejected by the owner**, and the reasoning is the important part: *"Unlike merit, which has a clear verifiable certificate, need is somewhat subjective. We try to get STR and salary slips and EPF and bills, and yet, it is still an estimate."*
**Rationale:** a band is a claim of precision. Merit earns one — there is a certificate behind it. Need does not: even with STR, salary slips, EPF and utility bills, the household figure is an ESTIMATE, and the engine's own honesty machinery says so (declared-unproven states, tolerance-based household checks, reviewer judgement at the gate). Rendering an estimate as a crisp bracket would launder that uncertainty into false confidence — and a funder would then allocate real money against a number the platform cannot actually stand behind. Prose that reads as somewhat subjective is the honest surface **because the underlying fact is somewhat subjective.**
**Trade-offs:** a funder comparing two cards has a harder, more qualitative judgement to make, and the card's merit signal stays sharper than its need signal — a known asymmetry, accepted as the honest one. Sorting/filtering the pool by need is correspondingly off the table.
**Revisit if:** need assessment becomes materially more certain — the owner's own condition: *"Perhaps when we are more certain, we can provide this with greater confidence."* Concretely: verified income coverage across the household rather than estimates, with the verdict engine expressing a confidence the platform is willing to publish.

## Money is OFF-platform until the CLBG exists — the wallet is a ledger of consequences, not of cash — 2026-07-26
**Decision / current reality (owner):** BrightPath has **no registered legal entity yet** — Suresh has engaged a company secretary to register a **CLBG**, expected to take a few months. Until then: **sponsors pay into Suresh's personal bank account, and he credits their wallet on the platform by hand.** In the owner's words: *"Real money is off the platform, but the consequences aren't."* Once the CLBG is registered, sponsors will donate **to the foundation via the platform**, auto-credited to their wallet, and allocate from there — opening up fractional giving, monthly donations into the foundation, and other combinations.
**What this settles for P4 (the org-admin wallet credit):** the admin-recorded credit is **not an edge case for one benefactor — it is THE funding path for the next few months, for every sponsor.** It must therefore be built as a first-class, audited flow, not a stopgap.
**Architectural instruction:** an admin-recorded credit and a future gateway donation must be **the same record with different provenance** — one `Donation`-shaped row carrying a `source` (admin-recorded / gateway) plus an external reference — never two parallel money systems that later have to be reconciled. The interim path is the permanent path with a different origin.
**Strengthens P4 open question 1 (maker-checker) toward YES:** in the interim arrangement the **same person receives the cash personally and records the credit**. A second signature is the only control that exists over that step, so it matters *more* now than it will after the CLBG. Open sub-question: who signs second when Suresh is the org's only finance-capable admin — the platform super (tamiliam) is the obvious stand-in, mirroring the payment-run chain's stand-in rule.
**Risks recorded honestly (owner is aware; these are for the record, not a challenge):** funds sit in an individual's personal account with no corporate ring-fence, so they are commingled with personal funds and their status in any personal misfortune is unclear; the platform meanwhile issues offers, agreements and award letters that create **real obligations to students against money it cannot see**; and reconciliation between wallet balances and the actual bank balance is entirely manual and trust-based. The mitigation available *in the system* is provenance: every credit carries who paid, how much, when, the external bank reference, and who recorded it — which is also exactly what makes the balances migratable into the CLBG's books once it exists.
**Future-state seams that already exist (so this is not a rewrite):** `Donation` already names toyyibPay as the intended gateway (mocked today); `Sponsorship` is 1:1 full-or-nothing but documents "many-sponsor plumbing underneath" for fractional giving; `StandingGift` already automates recurring **allocation** from a balance. What is genuinely absent is recurring **donation into the foundation** — that is gateway-side work, and it arrives with the CLBG.
**Revisit when:** the CLBG is registered. At that point: wire the gateway, switch the default donation source, keep the admin-recorded path for cheques/transfers, and re-scope the DPA question below.
**⚠ Knock-on for tenant #2 (Inspire):** a data-processing agreement needs **two legal entities**. With BrightPath's CLBG unregistered and HalaTuju itself org-homeless, there is currently **no entity on this side to sign a DPA**. This is a hard gate on onboarding Inspire's real applicant data, independent of Sprint E (erasure) being built. Raised 2026-07-26, ahead of the Inspire meeting.

## The wallet credit reuses the payments sign-off chain — no second control model — 2026-07-26
**Decision (owner):** *"For BrightPath, finance role is not yet filled. Once that is filled, that person will be the checker. For now, BrightPath checker is dark. It only has maker and approver."* The P4 wallet credit therefore adopts the **existing** payment-run chain rather than inventing its own controls: `draft → admin_signed → [finance_checked] → completed`, with the finance CHECK step **conditional and live-evaluated** via `payments.finance_check_required(organisation)` — never stored on the record.
**This closes P4 open question 1.** The answer is not "maker-checker yes/no" but "reuse the one we already built". Concretely: a credit is **recorded and signed by a maker**, optionally **checked by finance when the organisation has ≥1 active finance admin**, and **countersigned by an approver** before it becomes spendable balance. BrightPath today runs the two-step degraded chain (maker → approver, e.g. Poongulali → Suresh); the moment a finance admin is invited, the check arms itself — including for credits already sitting mid-chain, exactly as it does for a run already at `admin_signed`.
**Alternatives considered:** (a) a bespoke two-person control just for wallet credits — rejected: two competing sign-off models in one money module is how one of them rots; (b) requiring a finance admin before any credit can be recorded — rejected: it would block the funding path entirely until Sam joins, and the owner's design already treats the two-step chain as a legitimate degraded mode rather than a gap.
**Rationale:** the checker being dark is **not a workaround** — it is the designed degraded state, and its "never persisted, always live-evaluated" property is what makes activation retroactive rather than a migration. P4 inherits all of that by calling the same function. It also inherits the **three-distinct-signers pairwise** rule and the **stand-in rule** (the platform super may fill one slot as a stand-in, never both) — which is what answers "who signs second while Suresh is effectively the only admin".
...
**The view the owner describes already exists.** `sponsorship.sponsor_statement()` (R4) is exactly it: **donations INTO the trust** (one row each, with reference and timestamp) and **gifts OUT to students** (one row each, carrying only the anonymous `pool_ref` — never an identity), plus `total_in` / `total_out`. So P4 does **not** build a ledger; it (a) creates credits one-per-transfer with a real external reference in place of today's `reference='mock'`, and (b) makes the statement **programme-grouped**, since wallets are per (sponsor, programme) as of P2a.
**Alternatives considered:** a running top-up ledger reconciled periodically against the bank — rejected: it severs the 1:1 correspondence between a credit and a bank line, which is the entire audit trail in the pre-CLBG arrangement.
**Honesty requirement on the OUT side:** allocations are not monotonic. A `lapsed` or `cancelled` allocation stops holding and the amount returns to the balance (existing `Sponsorship` semantics). The statement must show returns as their own entries rather than silently reducing an earlier "out" line — a funder reading their statement should be able to see money come back, not just watch a total change.
**Revisit when:** the CLBG registers and the gateway is wired — a gateway donation becomes another row with `source='gateway'`, same shape, same statement. That is the point of the one-record design.
...
## The Sabah RM100,000 is a possibility, not a commitment — record nothing until inked AND transferred — Platform programme layer, 2026-07-26

**Decision:** The prospective ~RM100,000 Sabah gift is **not** to be represented anywhere in the platform — no `Programme` row, no `SponsorProgrammeMembership`, no `Donation` credit, no forecast figure on any surface — until **BOTH** (1) the Sabah programme is **inked** (agreement signed) **and** (2) the **money has actually changed hands**, with a bank reference available to cite as `external_reference`. Shipping P2a, P4a or P4b satisfies neither condition; those sprints only make the eventual recording *expressible and auditable*.

**Alternatives considered:** (a) Seed a Sabah `Programme` row ahead of time so the structure is ready and onboarding is a single step when the money lands — rejected: a seeded-but-unfunded programme is indistinguishable downstream from a live one. It would appear in programme pickers, membership vetting, the pool fence and any future statement or metering surface, and a restricted balance of zero reads as "a fund that exists and has been spent to nil", not "a fund that was never created". (b) Record the credit as a `draft` donation pending confirmation — rejected for the same reason plus a worse one: `draft` is a real state in a real sign-off chain, so it would sit in a maker-checker queue awaiting a signature for money that does not exist, training the operators to sign things that are not yet true.

**Rationale:** The owner's correction (2026-07-26): *"RM100,000 is a possibility. And it will not be recorded until the Sabah programme is inked, and the money transfers hands."* The programme layer was built **generically** — it carries no Sabah literal in production code — precisely so that a prospect can be onboarded when it becomes real without anything having been pre-committed on its behalf. Anticipatory data is the one way to undo that property.

**Trade-offs:** When Sabah does become real there is a short setup step (create the Programme, onboard the benefactor, credit the wallet) rather than a pre-warmed structure. That is a few minutes of admin work, weighed against a fictitious restricted fund sitting in a financial surface for an unknown number of months.
...
## The dark GROUND is designed; the dark TONES stay a reversal — Layer 1 F2a, 2026-08-31

**Decision:** the four tone ramps (positive / info / caution / critical) remain the light ramps
read from the other end, exactly as F1 built them. The ground ramp no longer is: its dark stops are
chosen to preserve ROLES — `0` the raised surface (`#1f2937`), `50` the page beneath it (`#111827`),
`100` wells and hover, `200`/`300` borders, `400` placeholder, `500` muted text — which shifts every
stop from `100` up by one position relative to a pure reversal.

**Alternatives considered:** (a) keep the pure reversal and repaint the components instead, giving
cards an explicit dark background — rejected, because it means editing 3,700 ground utilities by
hand, and the whole argument for a token layer is that tuning is an edit to one file; (b) add a
separate `--surface-raised` token beside the ramp — rejected as a second vocabulary for the same
idea, and every component already says `bg-ground-0` for a card; (c) leave it and call it a later
tuning pass — rejected because it is not a matter of taste: a card DARKER than its own page is
read as a hole, and every surface F3–F6 converts would be reviewed against a broken foundation.

**Rationale:** a tone is a signal and a signal only has to remain legible when reversed. The ground
is furniture, and furniture has roles that a value-preserving transform destroys. In light the
raised surface is white — the extreme of the ramp — so reversal necessarily sends it to the other
extreme, the bottom. There is no ordering of the light ramp for which the reversal gets this right.

**Trade-offs:** the dark ground is no longer derivable from the light one, so it cannot be
regenerated by a script and must be maintained by hand; `ground-900` and `ground-1000` now hold the
same white. Accepted: the ordering guard makes the property checkable even though the derivation is
gone, which is the stronger test anyway.

**Revisit if:** a designer does a full tone-tuning pass (the saturated dark tone fills are the known
next weak spot), or a third mode arrives — at which point the ramps want generating from a small
role table rather than typing.

## A progress fill carries the BRAND; a state badge carries its TONE — Layer 1 F2a, 2026-08-31

**Decision:** `FundingBar` and the Action Centre's progress bar are `primary` (brand). The "done"
medallions, the toast's success and error fills, and the status badges stay on their tones.
**Alternatives considered:** leaving the codemod's `info` for the funding bar (it was `blue`).
**Rationale:** F1's rule — *a filled control the user ACTS on carries the brand; a coloured surface
that INFORMS carries the tone* — does not settle a progress bar, which is neither. The deciding
question is whether the thing carries a SEMANTIC STATE. A progress fill does not; it is the product
measuring itself, and a tenant's colour should reach it. "This succeeded" does, and a tenant's
colour must never repaint it. The codebase had already made this call once (`bg-primary-500` in
...
## RESOLVED (F3b, 2026-08-31) — a pale brand tint used as a SURFACE had no dark counterpart — raised Layer 1 F3
...
## The brand's IDENTITY stop is the ruling; the other nine are furniture — Layer 1 F3b, 2026-08-31

**Decision:** `brandRamp()` takes a theme. In dark, the tints mix toward the page and the shades
toward white — the two ends swap — while **`--brand-500`, the tenant's actual colour, is
byte-identical in both modes**. `globals.css` gains a dark `--brand-*` block generated by that
function, and `branding-context` recomputes a tenant's ramp whenever the mode changes.

**This re-reads the 2026-07-29 ruling; it does not overturn it.** That rule — *a theme may never
write the tenant brand* — exists so switching mode cannot change WHOSE product you are looking at.
The identity is `500`. The other nine stops are furniture derived from it, and re-aiming furniture
at the surface it sits on changes no identity. The guard is now stricter than the ban it replaced:
`500` is pinned byte-for-byte across modes, where before nothing checked it at all.

**Alternatives considered:** (a) stop using brand tints as surfaces — 40 files, and it visibly
reduces how much of a tenant's colour reaches the product; (b) ship the pale patches — readable,
looks unfinished; (c) a second, hand-picked dark palette — **impossible**, because a tenant
supplies ONE colour at runtime, so anything hand-picked fits BrightPath and nobody else.

**Trade-offs:** the dark ramp cannot be eyeballed from the light one, and `branding-context` now
carries a MutationObserver. That observer is not optional: a tenant's ramp is written as INLINE
styles on `<html>`, which outrank the stylesheet's dark block, so without it a tenant would be
pinned to the light ramp in dark mode with no CSS remedy.

**Revisit if:** a tenant's colour is so light or so dark that the derived ends collapse (a
