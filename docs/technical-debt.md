...
- TD-081: **OCR signal-capturing improvements (backlog — to be expanded by the user).** The TD-080 fixes made documents *readable*; this tracks making the *reads* better. Known so far: (a) **marker-less names** (Chinese / Christian / mixed) have no A/L·A/P·BIN anchor, so `_extract_name` falls to the weaker "line-after-NRIC" tier — **candidate: a Gemini fallback** for the name (and optionally a blurry NRIC) when the deterministic read is low-confidence (cost-gated: only when no marker / a mismatch is flagged). (b) **Blurry-scan digit misreads** — e.g. YESWINDRAN's NRIC read `…0681` vs typed `…0661` (8↔6 on a low-res CamScanner PDF; the address had the same symptom). (c) **Clearer NRIC-mismatch guidance** — show the read vs typed number and tell the student to check the typed NRIC and/or re-upload a clearer photo (note: this is partly delivered by the S4 Action Centre). To be tackled as a dedicated "OCR signal capturing" pass after the verdict roadmap. (Logged 2026-06-02)
