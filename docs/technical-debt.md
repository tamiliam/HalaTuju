...
- TD-062: **Orphaned Supabase Storage blobs from pre-fix doc deletions.** Before today's single-instance-doc fix, the `DELETE /api/v1/scholarship/documents/<id>/` endpoint dropped the DB row but did NOT sweep the corresponding object in the `b40-documents` private bucket. Elanjelian's test account left ~3-4 orphan IC blobs (storage_paths matching the deleted doc IDs 1, 3, 4); other applicants who clicked Remove similarly leaked. The going-forward path is now clean (both DELETE and the new single-instance replace path call `storage.delete_objects`), so this only covers historical leaks. Cheap: write a one-shot management command that lists every object in the bucket via the Supabase Storage REST API and deletes any whose path doesn't correspond to an existing `applicant_documents.storage_path` row. Storage is cheap so this is low priority — flagging so we don't forget when we look at storage costs.
  ...
  - **Manual-run runbook (still PENDING — never run against prod; the ~3-4 Elanjelian app-3 orphan IC blobs persist).** The local `.env` has neither prerequisite by default (no prod DB conn, no service-role key — verified 2026-06-01). To run it needs BOTH: (a) the connection pointed at the **PROD** DB (pull `DB_*` from `gcloud run services describe halatuju-api`), and (b) the Supabase **service-role key** for Storage list/delete. **⚠️ FOOTGUN — orphans are computed as `bucket objects − ApplicantDocument rows in the CONNECTED DB`.** If the storage key is set but the DB points anywhere other than prod (local/sqlite/empty), EVERY real document is flagged as an orphan → `--apply` would delete the entire vault. **Always dry-run first and eyeball the output** (`DB references N blob(s)` should be the real prod count, orphan list should be ≈3-4 known paths, NOT "everything") before adding `--apply`. Low priority — storage is cheap, the going-forward delete path is clean, this is only historical leakage.
