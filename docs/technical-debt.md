...
- TD-081: **OCR signal-capturing improvements.** The TD-080 fixes made documents *readable*; this tracks making the *reads* better.
  ...
  - **✅ RESOLVED for the Academic/results-slip document (deployed 2026-06-02/03, `62339e9`+`177aed2`).** Fixed the "Entered 0 of 9" band-word bug (`academic_engine._split_band`), the clinical 3-check (`student_slip_check` → `ResultsSlipChecklist` Name/Subjects/Results + exam year), 3 specific Gopal verdicts; live-confirmed. Retro `docs/retrospective-check1-academic.md`.
  - **✅ RESOLVED for the Pathway/offer-letter document (deployed 2026-06-03, `3abe9a9`+`a0d997f`).** Differentiated facts (Name+IC checks + Programme/Institution/Issuer/Date/Address data points; expanded Gemini schema covering all post-SPM offer types; `pathway_engine.student_offer_check` + `OfferLetterChecklist`), PLUS the **AI-raised final-pathway confirmation** (migration `0038`). Live-confirmed across all 5 sample types. Retro `docs/retrospective-check1-pathway.md`.
  - **Residual (open) — INCOME is the last + hardest.** Not just "read the doc": income amount (per-capita vs household size → B40 test) + earner identity (the **parent IC**) + **relationship establishment** between earner and student — **father** auto-derivable via the student-IC patronymic, **mother** needs a **Birth Certificate** (a NEW doc type), **guardian** needs the guardianship letter — + utility bills as a soft hardship signal. Open policy question: today income-proof is a HARD completeness requirement (S23); should it relax to "provide if available" for informal-income families (officer judges on utilities + interview)? The dedicated Income sprint. (Logged 2026-06-02; Identity + Academic + Pathway resolved by 2026-06-03.)
