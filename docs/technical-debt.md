...
- TD-081: **OCR signal-capturing improvements.** The TD-080 fixes made documents *readable*; this tracks making the *reads* better.
  ...
  - **Residual (open):** the same Check-1 treatment for the **Academic / Income / Pathway** documents (results slip, payslip/EPF/STR, offer letter) — those use the supporting-doc Gemini doc-assist path already, but have not had a dedicated "good feedback on every read" hardening pass. Plus the **live billable smoke** of the IC Gemini path with a real low-confidence MyKad (user-run). (Logged 2026-06-02; Identity portion resolved same day.)
