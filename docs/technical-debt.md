...
- TD-074: **Sponsor-pool follow-ups (E2a, low priority, all behind the OFF flag).** (a) **Detail keyed by raw application id** — `GET /api/v1/sponsor/pool/<id>/` uses the DB row id (the card exposes `id` for the fetch). It's non-identifying and the endpoint returns 404 unless the student is currently pool-eligible, but it leaks row count/order to a vetted sponsor. If that matters, switch the API key to the opaque `pool_ref` (needs a ref→id resolver). (b) **The anon-profile generator is fed the student's free-text narrative** (aspirations/plans/family_context/daily_life/funding_note), which *could* contain a name/school/place. Mitigated three ways — the prompt forbids identifiers, an **admin reviews + publishes** before any sponsor sees it (human backstop), and the deterministic **allowlist card** is the hard boundary regardless of the blurb — but the blurb itself is model-trust + human-review, not structural. If a stronger guarantee is wanted later, add a post-generation identifier scan (e.g. check the blurb doesn't contain the profile's name/school tokens) before allowing publish. (c) **No filters yet** on the browse list (state/field) — fine for a small dummy pool; add when the pool grows. (Introduced v2.24.0, 2026-05-31)
