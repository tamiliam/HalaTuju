...
- TD-085: **▶ RE-SCOPED 2026-06-05 to TWO sprints (consent gate v2 + officer Documents-panel redesign). The original
  ...
  - **▶ S2 (next) — Documents-panel redesign (officer cockpit), Stitch-first:** coloured per-doc fact-labels, movable
    relationship, route+selection-aware ordering + missing-doc placeholders, fixes the `documentPill` earner-IC "Unread"
    bug.
