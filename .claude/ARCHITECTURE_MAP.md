...
**S3 (2026-09-10) added the sorter.** `apps/scholarship/spend_category.py` is the FOUR-RUNG LADDER, and every rule, threshold and prompt lives there and nowhere else: 1 `duitnow_type` → `transfer`; 2 whole-word keyword rules on the merchant name (126 of 288 real merchants); 3 the spend pattern (>=3 visits, median <= RM8) → `food`, **per TRANSACTION**; 4 Gemini on the rest. `manage.py sort_spending [--apply] [--all] [--no-ai]`; `ingest_spending --apply` calls the same ladder so the daily job stays ONE Scheduler entry; cron `spending-sort` is a DOOR, not a schedule.
⚠⚠ **THE RM20 PER-ROW CEILING IS LOAD-BEARING.** Rung 3 stores a merchant-level verdict (so rung 4 never pays to ask) but applies the ceiling per ROW. Six real payments across four genuinely-cheap food stalls sit above it — RM424 a merchant-level verdict would have filed as campus meals. `AL HUDHA ENTERPRISE` RM200 and `TEGUH ENIGMA (MATRIK 1)` RM97.70 are pinned by name against the real corpus.
⚠ **`ask_model(names)` TAKES MERCHANT NAME STRINGS AND NOTHING ELSE** — no amount, no student, no wallet, no date — through `vision._call_gemini_json`, the one Gemini seam. A signature test asserts the parameter list; the ten-code vocabulary is enforced in Python AFTER the answer returns. `transfer` is absent from the model vocabulary and from every keyword rule.
⚠ `category=''` (never sorted) and `category='unsorted'` (sorted, unplaceable) are DIFFERENT states. An unplaced row keeps `decided_by=''` so the next run reconsiders it for free. `decided_by='owner'` outranks all four rungs, at row and merchant level, and survives `--all`.
**S4a (2026-09-10) added the officer's screen.** `/admin/spending` (console, beside Payments) + `apps/scholarship/spend_report.py` (the read model AND the one write) + `AdminSpendingView` / `AdminSpendingCategoryView`. One row per **SHOP**, not per payment: four computed figures, the merchant table with a category control, per-student, what the model decided in the last fortnight, and the wallet gaps. en/ms/ta. No migration.
⚠⚠ **A CORRECTION WRITES AN `owner` VERDICT AGAINST THE SHOP AS WELL AS EVERY EXISTING ROW**, and the shop half is the one that matters: without it, next week's payment at that shop lands undecided and the ladder re-decides it, so the correction quietly stops applying. Found by a bite-check that came back SILENT — every test until then watched only the rows.
⚠ **THE FENCE IS ON THE QUERY** (`spend_report._txns`, `application__owning_organisation`), because this is a ROW question and an allowlist protects only a column (TD-201). `_SpendingBase` resolves the organisation once and refuses `no_org` rather than running unfenced. ⚠ A merchant VERDICT is global (a shop's category is a fact about the shop); the LIST is fenced, and so is who may SET it.
⚠ **`finance` IS REFUSED HERE THOUGH PAYMENTS ADMITS IT** — `_b40_scope` promises finance sees no student data beyond the Payments allowlist, and this screen carries names beside purchases. Backend, nav registry and page all refuse it.
⚠ **THE CATEGORY CONTROL IS A NATIVE `<select>`** — `TableFrame` clips, so a custom absolute panel in a cell is sliced off (the Intake years defect). The test asserts the ELEMENT. ⚠ **NO TIME OF DAY ANYWHERE** — a test asserts no `HH:MM` renders.
⚠ **`TestOrgFenceStaticGuard` NOW SCANS FOUR FILES** (`views_admin`, `spend_report`, `spend_category`, `spending_import`) with two FLOOR tests, because a name-scoped scan is blind to code moved one file sideways. `views_sponsor.py` has never been scanned — **TD-240**, named in `NOT_YET_SCANNED` with its reason. ⚠ A pragma must be the LAST line before its query; the guard reads 200 characters.
**S4b (2026-09-10) files the summary back to Drive.** `apps/scholarship/spend_summary.py` computes every figure of an import, asks Gemini for two or three sentences AROUND them, and writes Markdown into `…/06 Student Spending/Summaries` (`VIRCLE_SPENDING_SUMMARY_FOLDER`). Rides `ingest_spending --apply`; `--no-summary` opts out. No migration, no new schedule.
⚠⚠ **`_numbers_agree` IS THE GUARD, NOT THE PROMPT.** Prose containing any number that was not in the computed facts is DISCARDED and the figures are filed alone. ⚠⚠ **TWO LOCKS AGAINST READING OUR OWN OUTPUT:** a SUBFOLDER, and a filename that cannot match `sheets._SPENDING_FILENAME_RE` — asserted against the reader's own regex, imported, never a copy.
⚠ The prompt is told TODAY'S DATE and the prompt version is stamped into the DOCUMENT. The summary is INTERNAL: it names shops, names NO student, and no part of it reaches a sponsor. ⚠ Best-effort and LAST in the run — a Drive hiccup costs a document and nothing else, and the failure is printed rather than swallowed.
⚠ **`sheets.file_text_to_folder` IS NOW THE ONE DRIVE TEXT-WRITE PATH**; `file_csv_to_folder` delegates to it. `_find_or_create_folder` creates **only the last segment** of an output path we own — every earlier segment must exist, so a mistyped parent stays a loud failure.
**S5 (2026-09-10) added the sponsor card — THE ONLY SPONSOR-VISIBLE PART OF THE ARC.** `apps/scholarship/spend_sponsor.py` + a `spending` field on `SponsorMyStudentDetailSerializer` + `components/sponsor/SpendingCard.tsx`, filling the reserved panel on `sponsor/(portal)/my-students/[id]`. en/ms/ta. No migration.
⚠⚠ **`spend_sponsor.py` SHARES NOTHING WITH `spend_report.py` ON PURPOSE.** That one names merchants and students deliberately (the officer must correct them); this one serves an outsider. A merchant, a transaction id, a wallet and a purchase DATE are planted and asserted absent — at the function AND through the real endpoint. The payload is built one aggregate at a time, so a new `BursarySpendTxn` field cannot reach a sponsor unless somebody writes a line.
⚠ **`transfer` and `unsorted` are NEVER folded into "Other"** (owner) — the two honest categories. ⚠ **The "as at" stamp is the last IMPORT, not the last purchase** — the newest `txn_date` is the day the student last bought something. ⚠ **No card at all rather than an empty one**: four zeroes would claim they have spent nothing. ⚠ **Money crosses as a STRING** — a bare `Decimal` in a plain dict is rendered by DRF as a FLOAT.
⚠ The donut is hand-drawn SVG on the `category-N` swatches — **no chart library and no raw hex** (`theme.test.ts` reads SVG fills). The retired "coming soon" copy is gone from all three locales.
