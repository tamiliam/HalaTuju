...
export default function PreviewTab({ id, token, t }: {
  ...
  if (!data) return <p className="text-sm text-ground-500">{t('common.loading')}</p>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-ground-500">{t('admin.sponsors.terms.previewIntro')}</p>
      ...
      {doc.locale_used !== lang && (
        ...
        <p className="text-xs text-caution-700 bg-caution-50 border border-caution-200 rounded-lg px-3 py-2">
          {t('admin.sponsors.terms.previewFallback', { loc: lang.toUpperCase() })}
        </p>
      )}

      <article className="bg-ground-0 border border-ground-200 rounded-xl p-6 max-w-2xl">
        <h3 className="text-xl font-bold text-ground-900 mb-2">{doc.title || '—'}</h3>
        {doc.intro && <p className="text-ground-600 italic mb-6">{doc.intro}</p>}
        <div className="flex flex-col gap-5">
          {doc.sections.map((s) => (
            <section key={s.order}>
              <h4 className="font-semibold text-ground-900">
                <span className="font-mono text-xs text-ground-400 mr-2">{s.order}</span>
                {s.heading}
                {s.has_quiz && (
                  <span className="ml-2 text-[10px] uppercase tracking-wider text-info-600 bg-info-50 rounded px-1.5 py-0.5">
      ...
      {data.checkpoints.length > 0 && (
        <div className="max-w-2xl flex flex-col gap-3">
          <h4 className="text-sm font-semibold text-ground-800">
            {t('admin.sponsors.terms.checkpointCount', { n: String(data.checkpoints.length) })}
          </h4>
          {data.checkpoints.map((c) => (
            <div key={c.order} className="bg-ground-50 border border-ground-200 rounded-lg p-3">
              <p className="text-[11px] uppercase tracking-wider text-ground-500">{c.tag}</p>
              <p className="text-sm font-medium text-ground-900 mt-1">{c.question}</p>
              <ul className="mt-2 flex flex-col gap-1">
                {(c.options || []).map((o, oi) => (
                  <li key={oi} className={`text-sm ${
                    oi === c.correct ? 'text-positive-700 font-medium' : 'text-ground-600'}`}>
                    {oi === c.correct ? '✓ ' : '· '}{o}
                  </li>
                ))}
              </ul>
              {c.why && <p className="text-xs text-ground-500 mt-2">{c.why}</p>}
            </div>
          ))}
        </div>
      )}
