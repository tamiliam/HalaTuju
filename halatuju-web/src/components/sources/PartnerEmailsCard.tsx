...
export default function PartnerEmailsCard({ token, t }: {
  ...
  if (!data) {
    ...
    return error ? (
      <div className="rounded-lg border border-critical-200 bg-critical-50 p-4 mb-6 text-sm text-critical-600">{error}</div>
    ) : null
  }
  ...
  return (
    <div className="bg-ground-0 rounded-xl shadow-sm border overflow-hidden mb-6">
      <div className="px-4 sm:px-5 py-4 border-b">
        <h2 className="font-semibold text-ground-900">{t('admin.sources.emails.title')}</h2>
        <p className="text-sm text-ground-500 mt-0.5">{t('admin.sources.emails.subtitle')}</p>
      </div>
              ...
              <div className="grid grid-cols-[44px_minmax(0,1fr)] sm:grid-cols-[44px_minmax(0,1fr)_auto] gap-x-3.5 items-start px-4 sm:px-5 py-3.5">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {tpl.to_student && (
                      <span className="rounded-full bg-info-50 px-2 py-0.5 text-[11px] font-medium text-info-700 whitespace-nowrap">
      ...
      <div className="border-t bg-ground-50">
        <button type="button" onClick={() => setShowWho((v) => !v)}
          ...
          className="flex w-full items-baseline gap-2 px-4 sm:px-5 py-3 text-left text-sm text-ground-700 hover:text-ground-900">
        ...
        {showWho && (
          <div className="px-4 sm:px-5 pb-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wide text-ground-400">
                ...
                {data.organisations.map((o) => (
                  <tr key={o.id}>
                    <td className="border-b py-1.5">
                      {o.name} <span className="font-mono text-[11px] text-ground-400">{o.code}</span>
                    </td>
                    <td className="border-b py-1.5 text-right tabular-nums text-ground-500">{o.students}</td>
                    <td className="border-b py-1.5">
                      {o.is_house_org ? (
                        <span className="text-xs text-ground-400">{t('admin.sources.emails.houseOrg')}</span>
                      ) : o.qualifies ? (
                        <span className="text-xs text-positive-700">{t('admin.sources.emails.hasEmail')}</span>
                      ) : (
                        <span className="text-xs text-caution-700">{t('admin.sources.emails.noEmail')}</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <p className="mt-2.5 text-xs text-ground-400">{t('admin.sources.emails.recipientNote')}</p>
