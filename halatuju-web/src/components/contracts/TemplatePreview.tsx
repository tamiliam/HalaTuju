    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="text-xs text-ground-500">{t('admin.contracts.previewLocale')}</span>
          <LangTabs value={lang} onChange={(l) => { if (CLOCALES.includes(l)) setLang(l) }} />
        </div>
        <button type="button" onClick={openPdf}
          className="px-4 py-2 rounded-lg text-sm font-medium border border-ground-300 text-ground-700 hover:bg-ground-50">
      ...
      <iframe title="preview" srcDoc={html} className="w-full h-[70vh] rounded-xl border bg-ground-0" sandbox="" />
    </div>
  )
}
