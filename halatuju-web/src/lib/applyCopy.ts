...
// ⚠ NOTHING IN THIS MODULE MAY IMPORT A MESSAGE CATALOGUE (code health H17). `/scholarship/apply`
// imports `applyCard` from here, so a static `@/messages/*.json` line puts a megabyte of words
// into a student-facing page. `platformApplyCard`, the one function that genuinely needs all
// three, lives in `applyCopyPlatform.ts` and is reached through an `import()`.
