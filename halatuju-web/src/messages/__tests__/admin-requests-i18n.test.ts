/**
 * Guardrail — the Requests-space i18n keys (Sprint 15). Covers the new `admin.requests` namespace:
 * every t('admin.requests.*') referenced in source resolves in en.json, and the en / ms / ta key
 * sets under the namespace are identical (so a new key can't ship missing from ms/ta and render a
 * raw path). Mirrors admin-administration-i18n.test.ts.
 *
 * If this fails after you ADD a t('admin.requests.*') key: add it to all three message files. If it
 * fails after a RENAME: update both the code and the JSON.
 */
...
// Static keys under admin.requests ending at a closing quote. Dynamic template keys built from
// the requestStatus helpers — statusLabelKey/kindLabelKey/laneLabelKey — are covered by parity.
const re = new RegExp(`['"\`](admin\\.requests\\.[\\w.]+?)(?=['"\`])`, 'g')
const usedStatic = Array.from(new Set(captureGroup1(re, blob))).filter((k) => !k.endsWith('.'))

describe('admin.requests i18n hygiene', () => {
  test('every referenced admin.requests key resolves in en.json (no missing keys)', () => {
  ...
  test('at least a healthy number of keys are referenced (scanner not silently empty)', () => {
    expect(usedStatic.length).toBeGreaterThan(15)
  })

  test('en / ms / ta key sets are identical under admin.requests', () => {
