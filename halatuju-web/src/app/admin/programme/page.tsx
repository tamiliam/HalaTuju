...
// "What we ask for" — Layer 0 Sprint 5 (2026-08-30). Design of record: the Stitch screen
// approved 2026-07-29, CONTENT COLUMN ONLY — the shell (rail, breadcrumb, palette) comes from the
// app, never from the mock, and the mock's orange active row is not built (amber means caution).
//
// ⚠ THE CATALOGUE IS NOT A FENCE. This page reads and writes CONFIGURATION through one endpoint
// that is itself org-fenced; nothing here decides who may see what. `mayView` below only avoids
// rendering a page that would 403 — the endpoint is the authority.
//
...
export default function AdminProgrammeConfigPage() {
  ...
  // `load` depends on the token and the gate ONLY — never on `t`. A translator handle can be a
  // fresh function every render; depending on it here re-fires the fetch on each render, which
  // silently re-reads the server copy over an unsaved draft (found by the rendered test).
