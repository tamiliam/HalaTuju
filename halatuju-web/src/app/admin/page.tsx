...
export default function AdminDashboard() {
  ...
  if (error) {
    return (
      <div className="text-critical-600 mt-8 text-center">{error}</div>
    )
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-ground-0 rounded-lg p-6 shadow-sm border">
          <p className="text-sm text-ground-500">{t('admin.totalStudents')}</p>
        ...
        <div className="bg-ground-0 rounded-lg p-6 shadow-sm border">
          <p className="text-sm text-ground-500">SPM / STPM</p>
        ...
        <div className="bg-ground-0 rounded-lg p-6 shadow-sm border mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                <input
                  ...
                  readOnly
                  value={referralUrl}
                  className="flex-1 px-3 py-2 bg-ground-50 border border-ground-300 rounded-lg text-sm text-ground-700 select-all"
              ...
              <div className="flex gap-2">
                <a
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 bg-positive-fill text-positive-fill-ink text-sm rounded-lg hover:bg-positive-fill-hover transition-colors"
                >
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="bg-ground-0 p-3 border rounded-lg">
                <QRCode value={referralUrl} size={120} />
              </div>
              <p className="text-xs text-ground-400">{t('admin.scanToRegister')}</p>
            </div>
          </div>
        </div>
      )}
          ...
          <ul className="space-y-2">
            {data.top_fields.map((f) => (
              <li key={f.field} className="flex justify-between">
                ...
                <span className="text-ground-500">{f.count} {t('admin.studentsLabel')}</span>
              </li>
            ))}
          </ul>
        </div>
