...
const seenTone: Record<string, string> = {
  never: 'text-caution-700 font-semibold',
  dormant: 'text-caution-700',
  recent: 'text-ground-500',
  today: 'text-ground-500',
}

const statusBadge = (s: string) =>
  s === 'approved' ? 'bg-positive-100 text-positive-700'
    : s === 'pending' ? 'bg-caution-100 text-caution-700'
      // SUSPENDED is a heavier `caution`, not a fourth hue. It is a HOLD — nearer to `pending`
...
const actionStyle: Record<string, string> = {
  ...
  reject: 'bg-critical-600 hover:bg-critical-700',
  suspend: 'bg-caution-600 hover:bg-caution-700',
}
...
export default function AdminSponsorsList() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-ground-500 mt-1 mb-4">{t('admin.sponsors.desc')}</p>
              ...
              {paged.rows.map((s) => (
                <tr key={s.id} className="hover:bg-info-50/40 transition-colors align-top">
                  <td className="px-4 py-3 border-l-[3px] border-l-blue-500">
                    ...
                    {s.note && <div className="text-xs text-ground-500 mt-0.5 max-w-xs whitespace-pre-wrap">{s.note}</div>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-ground-500">{formatDate(s.created_at)}</td>
                  <td className="px-4 py-3">
