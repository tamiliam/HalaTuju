...
/** ⚠ EVERY SPONSOR RENDERS TWICE (2026-09-08): a phone card and a desktop table row. Which one
 *  you SEE is a CSS breakpoint, and jsdom applies none — so an assertion about ONE element has
 *  to say which rendering it means. These tests are about the table. */
...
describe('sponsors list columns', () => {
  it('shows Given and Last seen, and no Organisation column', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))

    expect(ui().getByText('admin.sponsors.colGiven')).toBeTruthy()
    expect(ui().getByText('admin.sponsors.colStudents')).toBeTruthy()
    expect(ui().getByText('admin.sponsors.colLastSeen')).toBeTruthy()
  ...
  it('links each sponsor to their own record', async () => {
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))
    expect(ui().getByText('Bharathan Nair').closest('a')!.getAttribute('href'))
  ...
  it('formats a given total and dashes a sponsor who gave nothing', async () => {
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))
    expect(ui().getByText('20,000.00')).toBeTruthy()
  ...
  it('shows how many students a sponsor is holding money for', async () => {
    ...
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))
    expect(ui().getByText('6')).toBeTruthy()
  ...
  it('says there is NO RECORD of a sign-in, not that they never came back', async () => {
    ...
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Chong Lee Min').length).toBeGreaterThan(0))
    const cell = ui().getByText('admin.sponsors.seen.never')
...
describe('sortable headers', () => {
  ...
  it('sorts by money as a number, so 20,000 leads 0.00', async () => {
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))
    fireEvent.click(ui().getByRole('button', { name: /colGiven/ }))
  ...
  it('flips direction when the same header is clicked twice', async () => {
    render(<AdminSponsorsList />)
    await waitFor(() => expect(screen.getAllByText('Bharathan Nair').length).toBeGreaterThan(0))
    const given = ui().getByRole('button', { name: /colGiven/ })
