  ...
  return (
    <details className="group rounded-xl border border-ground-200 bg-ground-0 transition-colors open:border-info-200 open:bg-info-50/30 hover:border-ground-300">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3.5 font-medium text-ground-900 [&::-webkit-details-marker]:hidden">
        <span>{item.q}</span>
        <svg className="h-5 w-5 shrink-0 text-ground-400 transition-transform duration-200 group-open:rotate-180 group-open:text-info-500"
          fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="m19 9-7 7-7-7" />
        </svg>
      </summary>
      <div className="px-4 pb-4 text-sm leading-relaxed text-ground-600">{item.a}</div>
    </details>
  )
}
...
function Section({ audience }: { audience: Audience }) {
  ...
  return (
    <section className="mt-8">
      <h2 className="flex items-center gap-2 text-base font-semibold text-ground-900">
        {AUDIENCE_LABEL[audience]}
        <span className="text-sm font-normal text-ground-400">· {items.length}</span>
      </h2>
        ...
        <div className="mt-5 flex items-center gap-2">
          <button type="button" onClick={() => setShowAll(false)}
            className={`rounded-full px-3 py-1 text-sm font-medium transition-colors ${
              !showAll ? 'bg-brand-fill text-brand-fill-ink' : 'bg-ground-0 text-ground-600 border border-ground-300 hover:bg-ground-50'}`}>
            For me
          </button>
