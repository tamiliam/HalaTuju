...
### The code-health arc — what has shipped (H1–H11; Phases 1–3 complete, Phase 4 begun)
...
`xapp` 45, `hot#1` unchanged. Six bite-checks, all six behaved.

**Measured, from two real `next build` runs:** `/` 483 → **259 kB**; `/scholarship/apply` 538 →
**314 kB**; `/profile` (worst before) 562 → **339 kB**; `/admin/scholarship/[id]` (worst after)
515 → **389 kB**. 74 of 88 routes shrank, 11 grew by 0.1–1.0 kB, 3 unchanged. The "shared by all"
chunk is 87.1 → 87.2 kB and always was going to be — **the catalogues were in the root LAYOUT
chunk, not the shared one**, and 87 kB is the FLOOR under every route.

**Five things to know before touching i18n delivery again:**
- ⚠ **`src/lib/messages.ts` is the ONE place `@/messages/*.json` may be written.** English is a
  static import (SSR, the first paint, and what `t()` answers with while another chunk is in the
  air); Malay and Tamil come through `import()` with **literal** specifiers. A computed
...
- ⚠ **A module is not a unit of consumption.** `lib/applyCopy.ts` held one pure function used by a
  ...
  it. Before adding a heavy import to a `lib/` module, ask which ROUTES import that module.
- ⚠ **Locale and catalogue are ONE piece of state in `i18n.tsx`, deliberately.** Two `useState`s
...
**Six things to know before touching `models` or `services` again:**
- ⛔ **`application_completeness` is in `services/completeness.py`. DO NOT CHANGE IT, IN ANY
  DIRECTION.** It was moved byte-identically and not touched. Its legacy document-type arm is
  deliberately more permissive and may only ever widen; replacing it un-submits students and nulls
  `requirements_snapshot` (H8). The module docstring says so where you will read it first.
- ⚠ **A `mock.patch('apps.scholarship.services.<dependency>')` string may no longer resolve.** The
  root's import block is gone, so `timezone`, `parse_date`, `ScholarshipApplication` and the five
  `send_*_email` senders are attributes of the module that READS them. H15 moved nine such strings
  (6 → `.confirmation`, 3 → `.assignment`) and **two `mock.patch.object(services, …)` targets**,
  which are the same hazard in a different shape — search for both. Two that resolve through the
  package at call time (`serializers_admin` re-imports inside the method; the cron dispatcher does
  `getattr(services, job)`) still work and were deliberately left alone.
- ⚠ **A ledger key had to MOVE, and in Python it always will.** `foo.py` cannot sit beside `foo/`
  — the package shadows the module — so H13's keep-the-path trick is a WEB-only trick. Both keys
  were relabelled through `_moved` (see `## Code standards`), the frozen `baseline` untouched and
  `BASELINE_SHA256` not re-pinned. **H16 must do the same.**
- ⚠ **Every new module name must be unique across `apps/scholarship/**`.** H15's first cut produced
  `models/money.py`, `models/contracts.py`, `models/org_requests.py` and `models/email_templates.py`
  when all four already existed at the top level. Python does not care; a source guard that
  allowlists by BARE FILE NAME does, and one did. Renamed to `funding`, `agreements`,
  `tenant_requests`, `comms_templates`. Check `ls apps/scholarship/*.py` at cut time.
- ⚠ **Before moving a file, grep for its path AND for every directory WALK that covers it.** A path
  string in a test fails loudly (six web drift tests did). A `glob('*.py')` that should have been
  `rglob` just covers less and keeps passing — `test_verdict_item_i18n.py` did exactly that.
  TD-276. Every walk in the suite should carry a floor that fails when it finds nothing.
- ⚠ **`AuditLoggerNameTest` now takes a LIST of packages** (`views_admin`, `services`), because a
  bite proved it did not follow the split: `getLogger(__name__)` in `services/assignment.py`
  turned nothing red, the one `assertLogs('apps.scholarship.services')` site included. **Add your
  package to `PACKAGES` and `FLOORS` when you split a file that logs**, and write the package name
  out in full in every submodule — never `__name__`.

**H12 SHIPPED 2026-09-20 — `views_admin/__init__.py` (5,093 lines) is 154 lines of re-export and
no code.** Moves only; **no behaviour changed and all twenty moved bodies are byte-identical** to
the lines they came from, proved by reading each back off disk against `git show HEAD:…`. The
nineteen remaining domains moved out as twenty modules (`reviewers` 475 · `verdict` 468 ·
`applications` 467 · `sponsors` 429 · `billing` 346 · `org_config` 341 · `lifecycle` 326 ·
`org_emails` 301 · `theme` 267 · `interviews` 246 · `spending` 210 · `sources` 203 ·
...
**Four things to know before touching `views_admin` again:**
- ⚠ **A `mock.patch('apps.scholarship.views_admin.<dependency>')` string no longer resolves.** The
  root's import block is gone, so `build_verdict`, `refine_sponsor_profile`, `timezone`,
  `send_request_info_email` and the rest are attributes of the module that READS them, not of the
  package. H12 moved 23 such strings (11 → `.verdict`, 6 → `.profiles`, 3 → `.verdict`, 2 →
  `.billing`, 1 → `.resolution`). A stale string now raises `AttributeError` rather than patching
  nothing, which is the failure you want. The reason is written in the root's own header.
- ⚠ **`interview_agenda_full` IS LIVE.** The roadmap, the H12 brief and this file all said it had
  "zero callers outside one test". It serves the `interview_agenda` field on every admin cockpit
  load (`serializers_admin.py:756`, typed at `admin-api.ts:1066`, rendered at `view.tsx:1052`).
  **It was not deleted and the phase's one permitted deletion was not spent.** The claim came from
  a symbol search, which answers "who imports this", not "who calls this".
- **EVERY module in the package must stay under 600 lines.** H4's ledger is frozen and cannot gain
  a member, and `views_admin/__init__.py` no longer has a budget line to fall back on — it left
  the oversize ledger at H12. A new endpoint goes in the domain module that owns it, or in a new
  one; never back in the root, which holds no code.
- **A submodule spells its logger name out** — `logging.getLogger('apps.scholarship.views_admin')`,
  never `__name__`. `AuditLoggerNameTest` (H11) enforces it. And ⚠ **do not quote `hot#1` as
  evidence** for anything in this package: `m_hotspots` counts fix commits by path and does not
  follow a rename.

**H13 SHIPPED 2026-09-20 — `admin-api.ts` (4,118 lines) is 241 lines of re-export and `api.ts`
(2,488) is 132.** Web only; no api file was touched. The bodies are 42 modules — 28 in
`src/lib/admin-api/` (max 443) and 14 in `src/lib/api/` (max 417), all under the 600-line standard
...
**⛔ READ THIS BEFORE THE NEXT api REFACTOR — H11 broke the WEB suite and no api gate could see
it.** `halatuju-web/src/lib/__tests__/officerGateDrift.test.ts` reads
`apps/scholarship/views_admin.py` by path (through `src/test/apiSource.ts`) to check the two
officer gates against the api's own source. H11 turned that file into a package and the test died
at import, taking 13 tests with it — including the guard on the irreversible org-admin reject
gate. The commit before H11 recorded **2,913 jest / 159 suites** green; from H11 until H13 the
tree was **2,900 / 158**, and H11 and H12 both passed every gate they ran, because backend-only
sprints run pytest and not jest. H13 repaired the path. **The systemic hole is TD-269: 31 of 159
web test files read source text and several read `halatuju_api/**`.** ⚠ **H15 moves `services.py`,
which `officerGateDrift` also reads — do TD-269 first, or H15 repeats H11 exactly.** Until it
exists, the manual rule is: **before moving or renaming any api file, grep `halatuju-web/src` for
its path.**

**H14 SHIPPED 2026-09-20 — `view.tsx` (3,599 lines) is 1,338 and `ScholarshipDocuments.tsx`
(1,914) is 825.** Web only; no api file was touched (pytest was run anyway: 7,021 / 3 skipped,
unchanged). Thirteen cockpit panels plus the shared furniture are 14 modules in
`src/app/admin/scholarship/[id]/view/` (max 346); the checklists and card furniture are 3 modules
...
- **`view.tsx` is still 1,338 lines and that is the floor for a moves-only sprint.** 784 lines of
  ...
  file over 600 lines is refused by the standard with no ledger line to record it, so a thin
  shell plus one big module is not available either. It goes under 1,000 when the Decision panel
...
a wide table (159 fields, 3 methods), not a fat class, so this is a file move; 310 importers, 3
patch sites, and migrations address models by label, not by file. `services.py` (2,946) splits on
its existing seams (blockers, assignment, decline, confirmation, consent) behind a re-exporting
shim. Acceptance: **`makemigrations --check` clean — a models move that generates a migration is
a failed move** — and pytest green. Re-estimated **~8h** on H14's measured cost. Phase 4's rule is
still **moves only**: the proof is a full green suite plus bodies byte-identical to the lines they
came from. ⚠ **Take BOTH baselines yourself before touching anything**, and ⚠ **run BOTH suites at
the end whatever you touched** — see the TD-269 warning above; `officerGateDrift.test.ts` reads
`services.py` by path from the WEB tree, so H15 can repeat H11 exactly.
