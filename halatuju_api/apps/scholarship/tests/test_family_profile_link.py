...
from apps.scholarship.tests.factories import (
    TEST_JWT_SECRET as _TEST_JWT_SECRET, make_application, make_cohort, make_student,
)
...
class TestCopyRoster(TestCase):
    def test_copy_is_field_for_field(self):
        prof = make_student(supabase_user_id='cp1', **_ROSTER)
        app = make_application(
            'submitted', cohort=make_cohort(code='cp', name='B40', year=2026), student=prof)
...
class TestStorySaveMirrorsToProfile(TestCase):
    ...
    #: The stage each status under test is reached at. `rejected` is the DECIDED case whose
    #: copy freezes; the rest are live.
    _STAGE_FOR = {'profile_complete': 'profile_complete', 'shortlisted': 'shortlisted',
                  'recommended': 'recommended', 'rejected': 'rejected',
                  'submitted': 'submitted'}

    def _app(self, status):
        prof = make_student(supabase_user_id=f'{status}-{self.id()}')
        return make_application(self._STAGE_FOR[status], cohort=self.cohort,
                                student=prof), prof
...
class TestProfilePutMirrorsToOpenApp(TestCase):
    ...
    def setUp(self):
        ...
        self.profile = make_student(supabase_user_id='put-1', nric='080115-05-0132')
    ...
    def test_profile_edit_flows_into_open_app(self):
        app = make_application('shortlisted', cohort=self.cohort, student=self.profile)
    ...
    def test_profile_edit_does_not_touch_decided_app(self):
        app = make_application('rejected', cohort=self.cohort, student=self.profile,
...
class TestProfileGetSurfaces(TestCase):
    ...
    def test_get_returns_family_fields(self):
        prof = make_student(supabase_user_id='g1', **_ROSTER)
        r = self._get(prof)
        self.assertEqual(r.status_code, 200, r.content)
    ...
    def test_merit_score_none_without_grades(self):
        prof = make_student(supabase_user_id='g5', grades={})
        r = self._get(prof)
...
class TestPathwayProfileLink(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.cohort = make_cohort(code='pw', name='B40', year=2026)

    def setUp(self):
        self.client = APIClient()
        self.profile = make_student(supabase_user_id='pw-1', nric='080115-05-0132')
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {_token(self.profile.supabase_user_id)}')

    def test_copy_pathway_field_for_field(self):
        prof = make_student(supabase_user_id='pw-cp', **_PATHWAY)
        app = make_application('submitted', cohort=self.cohort, student=prof)
    ...
    def test_profile_pathway_edit_does_not_touch_decided_app(self):
        app = make_application('recommended', cohort=self.cohort, student=self.profile,
    ...
    def test_get_returns_pathway_fields_from_profile(self):
        prof = make_student(supabase_user_id='pw-get', **_PATHWAY)
...
class TestIdentityVerified(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.cohort = make_cohort(code='iv', name='B40', year=2026)
    ...
    def _ic(self, prof, vname, vnric):
        app = make_application('shortlisted', cohort=self.cohort, student=prof)
