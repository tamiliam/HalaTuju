...
from apps.scholarship.tests.factories import make_application, make_org, make_student
...
def _cohort(org=None, code='c-2026', **kw):
    """⚠ DELIBERATELY PROGRAMME-LESS. This file pins the `owning_organisation` derivation
    itself, so a cohort here carries the org under test and nothing else — the gift layer is
    one level down and is pinned by `test_programme_layer.py`."""
    from apps.scholarship.tests.factories import make_cohort
    return make_cohort(programme=None, code=code, name='Test Cohort', year=2026,
...
def _application(cohort, profile):
    """An application at the first stage the product reaches — the row exists, nothing has run.
    `make_application` performs exactly one `objects.create`, which
    `test_derivation_uses_no_extra_query_when_cohort_loaded` counts."""
    return make_application('submitted', cohort=cohort, student=profile)


class TestOwningOrgDerivation(TestCase):
    def test_derives_from_cohort_on_direct_create(self):
        org = _org()
        app = _application(_cohort(org), _profile())
    ...
    def test_bare_cohort_stays_none_no_crash(self):
        ...
        app = _application(_cohort(org=None), _profile())
    ...
    def test_set_once_not_overwritten_on_resave(self):
        ...
        app = _application(cohort, _profile())
...
class TestBackfillMechanism(TestCase):
    ...
    def test_backfill_query_fills_every_row_with_an_owned_cohort(self):
        ...
        org = _org()
        owned = _application(_cohort(org, code='owned'), _profile('u-o'))
        bare = _application(_cohort(None, code='bare'), _profile('u-b'))
...
class TestProtect(TestCase):
    def test_org_delete_protected_by_owning_application(self):
        ...
        org = _org()
        _application(_cohort(org), _profile())
