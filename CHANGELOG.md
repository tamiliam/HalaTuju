...
- **Check 2 — submission review → queries → SLA → claim-gated profile (Sprints 2–5).** The post-submit pipeline that
  ...
  `ResolutionItem.kind` += `clarify`/`human`, `source` += `check2`, migration 0044; `check2_queries.py`): factual,
  ...
  default 5, `ScholarshipApplication.query_reminder_at`, migration 0045): `is_ready_for_assignment` = no open clarify
  ...
  clean; i18n parity 2105; migrations through 0045.
