...
def render_ai_report_page(user, t):
    ...
    with col1:
        ...
        try:
            ...
            st.download_button(
                label=t.get('btn_download_pdf', "📄 Download PDF"),
                data=pdf_buffer,
                file_name=fname,
            ...
            print(f"PDF Error: {e}")
            st.button(t.get('btn_download_pdf', "📄 Download PDF"), disabled=True, use_container_width=True)
    ...
    with col2:
        ...
        st.link_button(
            label=t.get('btn_share_wa', "📲 Share with Parent"),
            url=whatsapp_url,
            use_container_width=True
        )
    ...
    with col3:
        ...
        if st.button(t.get('btn_print_report', "🖨️ Print Report"), use_container_width=True, key="btn_print"):
    ...
    if st.button(t['btn_back_dash'], use_container_width=True, type="primary"):
        st.session_state['view_mode'] = 'dashboard'
        st.rerun()
    ...
    st.markdown(f"<div style='text-align: center; color: #888; font-size: 0.8em; margin-top: 20px;'>{t.get('lbl_generated_by', 'Generated by {counsellor} • Engine: {model}').format(counsellor=c_name, model=model_name)}</div>", unsafe_allow_html=True)
...
if auth_status:
 ...
 # --- PAGINATED LIST VIEW ---
            ...
            if not filtered_raw:
                ...
                # Show Result Count
                st.caption(t.get('filter_count', "Showing {shown} of {total} courses.").format(shown=len(filtered_raw), total=len(grouped_courses)))
